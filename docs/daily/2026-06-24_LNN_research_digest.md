...
- **今日新增最高优先级：read_now + experiment** — [Topological Neural Dynamics](https://arxiv.org/abs/2606.21295v2) 将序列建模从 layer-wise dynamics 推到 neuron-wise graph dynamics，并以 CfC 作为强基线之一；已生成独立研读报告：[[docs/reports/Topological_Neural_Dynamics_2606.21295_研读报告.md]]。
- **控制/机器人方向：read_now + experiment** — [GazeLNN](https://arxiv.org/abs/2606.20491v1) 已补独立报告：[[docs/reports/GazeLNN_Fast_Human_Attention_Prediction_2606.20491_研读报告.md]]；[heimdilon/sncp-ppo-crowdnav](https://github.com/heimdilon/sncp-ppo-crowdnav) 指向 CrowdNav，适合进入 `sncp_policy_lite` / TurtleBot3 轻量验证队列，但需要先确认代码与数据可复现性。
- **应用扩展方向：watch** — [FlowFake](https://arxiv.org/abs/2606.19579v1) 把 liquid networks 用于音频 deepfake 跨数据集泛化；[MA-GLTC](https://arxiv.org/abs/2606.15807v1) 已有独立报告，继续作为图时空 LNN 主线追踪。
- **开源生态方向：repo_analyze** — [raminmh/CfC](https://github.com/raminmh/CfC) 仍是 CfC 官方/核心实现锚点；[maximecb/bebelm](https://github.com/maximecb/bebelm) 和 [RacineAI-comp/lfm2-embedding-fp8-engine](https://github.com/RacineAI-comp/lfm2-embedding-fp8-engine) 值得分别用于 CPU-only LFM2.5 与 FP8 embedding engine 可行性观察。
- **模型生态方向：experiment** — Hugging Face 今日出现多个 LFM2.5 350M / 1.2B / 8B-A1B 派生模型，其中 [LiquidAI/LFM2.5-Embedding-350M-GGUF](https://huggingface.co/LiquidAI/LFM2.5-Embedding-350M-GGUF) 与 [LiquidAI/LFM2.5-ColBERT-350M-GGUF](https://huggingface.co/LiquidAI/LFM2.5-ColBERT-350M-GGUF) 继续适合 Jetson / MLX / GGUF 检索任务验证。

### 候选 triage

| 状态 | 条目 | 理由 | 下一步 |
|---|---|---|---|
| read_now / experiment | [Topological Neural Dynamics](https://arxiv.org/abs/2606.21295v2) | 与 CfC baseline 直接对比，提出 neuron-wise + graph topology 结构命题；参数量 0.36M 下 Pong 平均连续接球数 17.47，高于 CfC 5.41。 | 已写研读报告；设计 Graph-CfC smoke test。 |
| read_now / experiment | [Fast Human Attention Prediction / GazeLNN](https://arxiv.org/abs/2606.20491v1) | MobileNetV3 + CfC 自回归 scanpath prediction，0.61 GFLOPs、6.84 ms/frame，并在 Jetson Orin NX 上接入主动相机导航。 | 已写研读报告；优先做 GazeLNN-lite synthetic heatmap + Jetson latency probe。 |
| watch | [FlowFake](https://arxiv.org/abs/2606.19579v1) | liquid networks 用于音频 deepfake 跨数据集泛化，属于鲁棒时序分类应用。 | 观察是否有代码；若有公开特征提取流程再进入复现实验。 |
| watch | [MA-GLTC](https://arxiv.org/abs/2606.15807v1) | 图时空 + Liquid Time-Constant，已在本仓报告覆盖。 | 维持图时空方向索引，不重复深读。 |
| repo_analyze | [heimdilon/sncp-ppo-crowdnav](https://github.com/heimdilon/sncp-ppo-crowdnav) | 今日更新，PPO + LTC crowd-aware navigation；贴近 NCP/LTC 控制路线。 | 检查 Colab、依赖、TurtleBot3 仿真路径。 |
| repo_analyze | [maximecb/bebelm](https://github.com/maximecb/bebelm) | Rust CPU-only LFM2.5-8B-A1B 推理，Star 增长到 59；适合边缘推理生态观察。 | 记录模型格式、内存峰值、token/s 与许可证。 |
| experiment | [LiquidAI/LFM2.5-Embedding-350M-GGUF](https://huggingface.co/LiquidAI/LFM2.5-Embedding-350M-GGUF) | 官方 GGUF embedding，下载量继续上升，和本仓 Jetson/边缘检索目标一致。 | 加入 `analysis/jetson/` 量化推理候选。 |
...
| 2026-06-23 | [raminmh/CfC](https://github.com/raminmh/CfC) | 1048 | Python | Closed-form Continuous-time Neural Networks |
...
| 更新 | 模型 | 下载 | Likes | 任务 |
|---|---|---:|---:|---|
| 2026-06-24 | [OrdenWills/LFM2.5-350M-home-assistant-sft](https://huggingface.co/OrdenWills/LFM2.5-350M-home-assistant-sft) | 3372 | 4 | text-generation |
| 2026-06-24 | [LLM-OS-Models/LFM2.5-8B-A1B-Raw-ECHO-RLVR-GRPO-Adapters](https://huggingface.co/LLM-OS-Models/LFM2.5-8B-A1B-Raw-ECHO-RLVR-GRPO-Adapters) | 0 | 1 |  |
| 2026-06-24 | [PersonaPO/lfm25-8b-a1b-structdpo-bigchat](https://huggingface.co/PersonaPO/lfm25-8b-a1b-structdpo-bigchat) | 0 | 0 | text-generation |
...
| 2026-06-23 | [vsuhas9/LFM2-VL-450M-32](https://huggingface.co/vsuhas9/LFM2-VL-450M-32) | 0 | 0 | image-text-to-text |
...
| 2026-06-22 | [LiquidAI/LFM2.5-ColBERT-350M-GGUF](https://huggingface.co/LiquidAI/LFM2.5-ColBERT-350M-GGUF) | 2174 | 21 | sentence-similarity |
...
## PDF 归档
- `papers/daily/2026-06-24/2026-06-19_Topological_Neural_Dynamics_2606.21295.pdf`
- `papers/daily/2026-06-24/2026-06-18_GazeLNN_Fast_Human_Attention_Prediction_2606.20491.pdf`

## 建议动作
- 优先把 [[docs/reports/Topological_Neural_Dynamics_2606.21295_研读报告.md]] 转化为 Graph-CfC 最小实验：在现有 CfC cell 外增加稀疏有向图交互，先跑 synthetic nonstationary sequence，再考虑 Pong imitation。
- 将 [[docs/reports/GazeLNN_Fast_Human_Attention_Prediction_2606.20491_研读报告.md]] 转化为 GazeLNN-lite 实验：先做合成 fixation heatmap 自回归与 Jetson 延迟测试，不直接接入真机闭环。
