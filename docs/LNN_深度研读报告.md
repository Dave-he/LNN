...
### [2026-08-27] SDE-LNN — Stochastic Liquid Deformation Fields: 把 CfC 闭式解解读作 one-step SDE（4D Gaussian Splatting 续作）
- **独立报告**：[[docs/reports/Stochastic_Liquid_Deformation_Fields_SDE_CfC_2608.28702_研读报告.md]]
- **核心问题**：承接 [[docs/reports/Liquid_Neural_Networks_3DGS_Deformation_Field_2606.07670_研读报告|Liquid_NN_3DGS_Deformation_Field_2606.07670]] 的 deterministic CfC 形变场——CfC 是 LTC ODE 的"闭式（deterministic）极限"，丢弃了 LTC 本身的扩散项，而 liquid 网络的鲁棒性恰恰归因于这种随机性。**能否在不引入 SDE solver 的前提下，把扩散项"架构性"地放回 CfC？** 同时这也是 D-NeRF / NeRF-DS 这类密集插值基准是否真正能体现 noise 项价值的实证问题。
- **方法论**：把 cell 内 elapsed-time signal $\tau$ 进入 cell 的唯一通道（sigmoidal time gate 的 affine pre-activation $s = W_a z \tau + W_b z$）视为 one-step Itô 过程，加 Euler–Maruyama 增量 $\lambda \varepsilon$（$\varepsilon \sim \mathcal{N}(0, I)$）：$\sigma_\tau = \sigma(W_a z \tau + W_b z + \lambda \varepsilon)$。$\lambda = 0$ 时严格退化为 standard CfC（exact reduction），**完全共享参数 / FLOPs / 推理路径**——使 noise 项的实证贡献可被干净归因。Noise 仅在训练时采样一次/forward，test-time 关闭 → 推理与 deterministic CfC 完全相同（区别于 Neural SDE）。架构：D=6 cells, W=128, backbone depth 2, GELU；D-NeRF 40k / NeRF-DS 20k iter，单 GPU。
- **关键成果**：D-NeRF 8 场景中 **6/8 最佳 PSNR**，均值 **38.28 vs 38.19 dB**；**Hook +1.24 dB**、Stand Up +0.37 dB、Hell Warrior +0.38 dB——最高频大幅运动场景上 liquid 优势最显著，与"noise 抹平 τ 抖动"假设一致。**NeRF-DS 7 场景均值 23.73 vs 23.82 dB**（pose noise 主导，noise 项反而略掉点）；Bell +0.07 / Press +0.14 是难操作场景的最佳。NeRF-DS 配置下 liquid 形变场 **比 D-3DGS MLP 小 39%**（0.32M vs 0.52M）。**Noise-level sweep**（λ ∈ {0, 0.05, 0.10, 0.20}）：D-NeRF λ=0/0.05 持平（38.28 dB），更大 λ 抹掉真实运动；NeRF-DS **λ=0 已是最佳**（23.83 dB），任何正 λ 都略差。Run-to-run 噪声 σ ≈ 0.05 dB（3 次重训），单次差距多数在噪声以内，但 sweep 单调趋势独立支撑结论。
- **局限**：D-NeRF / NeRF-DS 这类密集监督插值任务 under-tests liquid 鲁棒性（supervision jitter 太小，noise 项无 error 可抹）；仅做 first-order, single-step, gate-level 注入（未在 hidden state 加扩散）；只做一次 constant λ，未给 input-dependent / annealed schedule；Bell/Press 真实场景只有 +0.07/+0.14 dB，缺乏定性可视化；未跑 inference-time Monte-Carlo 不确定性量化。
- **对本仓**：**SDE 视角是 liquid 鲁棒性的可引用机制化解释**，可作为后续 CfC / liquid SDE 改动的引用基础；**λ 作为 free knob** 的设计哲学值得在 [[Multi-Rate_MoE_Accelerating_LNN_Training_2606.12240_研读报告|PLAN / MR-MoE 训练加速]]（2606.12240）等 liquid 训练范式中复用（annnealed λ → 0 = curriculum）；可作 `lnn/core/variants.py` 的新入口 `StochasticCfCCell(lambda_init=0.05)`；**复现成本极低**（约 5–10 行代码，单标量 λ），优先复制 D-NeRF Hook (+1.24 dB) 与 NeRF-DS Bell/Press 作为 sanity check。**Verdict**: TARGET-POSITIVE（机制清晰化 + 诚实负结果 + free knob / 1 行级复现成本），密集插值基准 NEGATIVE-WITH-NUANCE（noise 项不能涨点，但解释力与可调性依旧成立），鲁棒性真实验证（jittered timestamps / pose noise / few-view）DEPENDENT-WITH-NUANCE（这是 noise 项真正该有用的场景，留作未来工作）。
- **与前作 2606.07670 对照**：2606.07670 用 CfC **drop-in 替换 MLP 形变场**并报告 "新 SOTA"；2608.28702 在同一框架上**保留 CfC + 加 gate-level noise**，报告 "诚实负结果"——立场从 benchmark 拼分转为机制清晰化。两条路径应共同作为 CfC × D-3DGS 的标准 baseline。
...
- **2026-09-05**：[[docs/daily/2026-09-05_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 17 个。
...
- **2026-08-27**：[[docs/daily/2026-08-27_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 21 个。
- **2026-08-25**：[[docs/daily/2026-08-25_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 17 个。
- **2026-08-26**：[[docs/daily/2026-08-26_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 17 个。
- **2026-08-24**：[[docs/daily/2026-08-24_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 17 个。
- **2026-08-23**：[[docs/daily/2026-08-23_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 18 个。
- **2026-08-22**：[[docs/daily/2026-08-22_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 18 个。
- **2026-08-21**：[[docs/daily/2026-08-21_LNN_research_digest.md|每日追踪]]，候选论文 25 篇，仓库 40 个，模型 18 个。
