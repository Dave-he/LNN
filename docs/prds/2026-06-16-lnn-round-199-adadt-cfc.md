# PRD #10-161 — Round 199 — Content-Aware Adaptive dt for CfC
...
Rounds 192-198 explored regularization and integration schemes.
Round 198 RK4 was first per-dataset win since r192 (sin -19%).
Round 199 pivots to a different axis: **content-aware adaptive
dt** (the current CfC uses fixed dt=1.0).

Inspired by Hyena Edge (Liquid AI, April 2025) which uses long
convolutions as a replacement for attention, suggesting that
content-modulated dynamics can be a powerful inductive bias.

## Goal

Test if learning the dt at each timestep based on input content
helps CfC on time series prediction.
...
1. `cf`: default closed-form, fixed dt=1.0 (r187 baseline)
2. `adadt`: content-aware adaptive dt, dt_max=2.0
...
Per-dataset:
- sin_irr: 0.0381 → 0.0376 (-1.3%, neutral — mixed seeds)
- structured_irr: 0.0001 → 0.0001 (0%)
- random_irr: 0.0834 → 0.0910 (+9.1%)

## Verdict

**NEGATIVE (53rd)** — content-aware adaptive dt doesn't help
on average. Hurts random, neutral on sin/structured.
