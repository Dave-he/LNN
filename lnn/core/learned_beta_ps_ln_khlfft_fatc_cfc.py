"""LearnedBetaPS+LN+Khl+FFT+FATC-CfC (PRD #10-187, Round 225, 2026-06-17).

Frequency-Adaptive Time Constants (FATC): the dominant FFT
frequency of the hidden state modulates the per-feature
time_scale. High frequencies → smaller time_scale (faster
response); low frequencies → larger time_scale (slower).

Couples the spectral axis (r210-r224) with the core CfC
time-constant mechanism.
"""
...
class FATCSpectralCfCCell(nn.Module):
    """CfC cell with frequency-adaptive time constants + 4-scale spectral."""
    ...
    def __init__(
        ...
        # Base time_scale (per-feature)
        self.time_scale = nn.Parameter(torch.ones(hidden_size))
        # FATC: per-feature modulation of time_scale based on freq
        # freq_proj: dominant freq index -> per-feature scale factor
        # n_freq = hidden_size // 2 + 1
        n_freq = hidden_size // 2 + 1
        self.freq_to_scale = nn.Linear(n_freq, hidden_size)
        # Mix between base and adaptive
        self.fatc_mix = nn.Parameter(torch.tensor(0.5))

        # 4-scale spectral (same as r216)
    ...
    def _adaptive_time_scale(self, h):
        ...
        # Add a small floor to avoid div by zero
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        # FATC: per-feature time scale from FFT of h
...
class FATCSpectralCfCStackedNetwork(nn.Module):
    """Stacked FATC + 4-scale spectral CfC."""
...
def make_lbps_lnkhlfft_fatc_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-scale spectral + FATC time constants + bias + dropout p=0.2."""
    return FATCSpectralCfCStackedNetwork(
