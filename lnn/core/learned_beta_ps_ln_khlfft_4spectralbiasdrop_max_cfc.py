"""LearnedBetaPS+LN+Khl+FFT+4ScaleSpectralBiasDropout+Max-CfC (PRD #10-182, Round 220, 2026-06-16).

Like r216 (4-scale + bias + dropout) but combines the 4 scales
via **max** instead of **average** (across scales per channel).

g_combined_max = max(g1, g2, g3, g4)  # element-wise across scales

Hypothesis: max combination picks the most informative scale
per channel (winner-take-all) instead of averaging.
"""
...
class FourScaleSpectralBiasDropMaxCfCCell(nn.Module):
    """CfC cell with 4-scale spectral + bias + dropout + max combination."""
    ...
    def _4scale_spectral_max(self, h):
        ...
        # MAX combination (vs avg in r216)
        return torch.stack([g1, g2, g3, g4], dim=0).max(dim=0).values
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        g = self._4scale_spectral_max(h_t)
...
class FourScaleSpectralBiasDropMaxCfCStackedNetwork(nn.Module):
    """Stacked 4-scale spectral + bias + dropout + max combination CfC."""
    ...
    def __init__(
        ...
        for l in range(num_layers):
            ...
            self.cells.append(
                FourScaleSpectralBiasDropMaxCfCCell(
...
def make_lbps_lnkhlfft_4spectralbiasdrop_max_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-Scale Spectral + bias + dropout + MAX combination."""
    return FourScaleSpectralBiasDropMaxCfCStackedNetwork(
...
__all__ = [
    "FourScaleSpectralBiasDropMaxCfCCell",
    "FourScaleSpectralBiasDropMaxCfCStackedNetwork",
    "make_lbps_lnkhlfft_4spectralbiasdrop_max_5_3_2",
]
