"""HybridBeta-XH-CfC (Scalar β on h + Per-Feature β on x) (PRD #10-125, Round 163, 2026-06-15).

Cross of rounds 161 (best for sin) and 162 (best for structured):
- Per-feature learned β on x-side (round 157/162)
- SCALAR fixed β on h-side (round 161)

Tests if the two best mechanisms can be combined for BOTH best
sin AND best structured simultaneously.

Mechanism::

    # Per-feature learned β on x-side (round 162):
...
Hypothesis from round 162: per-feature β on the hidden state hurt
sin (overfits on uniform-frequency data). The natural test: use
per-feature β on x-side (helps structured) + scalar β on h-side
(helps sin). Best of both worlds.
"""
...
class HybridBetaXHCfCCell(nn.Module):
    """HybridBeta-XH-CfC cell: per-feature learned β on x +
    scalar fixed β on h multi-scale EMAs.
    ...
    Args:
        ...
        betas_h: list of scalar β values for h-side EMAs.
    ...
    def __init__(
        ...
        if betas_h is None:
            ...
            assert len(betas_h) == Kh, f"len(betas_h)={len(betas_h)} != Kh={Kh}"
        ...
        # Aug input size: D + Kx * D = (Kx+1) * D
        aug_input_size = (self.Kx + 1) * input_size
        # Aug hidden size: H + Kh * H = (Kh+1) * H
        ...
        # SCALAR fixed β on h-side (no parameters).
    ...
    def forward(
        ...
        # Update h-side EMAs with SCALAR fixed β.
        emas_h_new = [
            self.betas_h[k] * emas_h[k] + (1.0 - self.betas_h[k]) * h
...
class HybridBetaXHCfCStackedNetwork(nn.Module):
    """Stacked HybridBeta-XH-CfC network (PRD #10-125).

    Args:
        input_size: input feature dimension.
        hidden_size: hidden dimension (per layer).
        output_size: output feature dimension.
        num_layers: number of stacked HybridBeta-XH-CfC cells.
        Kx: number of x-side EMA channels.
        Kh: number of h-side EMA channels.
        betas_h: list of scalar β values for h-side EMAs.
        mode_x: 'diff' or 'concat' for x-side.
    ...
    def __init__(
        ...
        self.Kx = Kx
        self.Kh = Kh
        self.betas_h = betas_h if betas_h is not None else [0.9] * Kh
        self.mode_x = mode_x
        self.mode_h = mode_h
        self.beta_init = float(beta_init)
