"""LearnedBetaPS+LN+Khl+FFT+SpectralDropoutLow-CfC (PRD #10-167, Round 205, 2026-06-16).

Variation of r203 with lighter dropout (p=0.2 vs 0.3).
Wraps round 187's winner (lbps_lnkhlfft_5_3_2) with spectral
gating + dropout at p=0.2 (vs r203's p=0.3).

Hypothesis: lighter dropout may preserve more sin structure
and recover random more (smaller p = closer to baseline).
...
Mechanism:
    For each timestep t:
        ...
        # Spectral gating with dropout (round 203, p=0.2 here)
        H = FFT(h_t)
        magnitude = |H|
        mask = sigmoid(linear(magnitude))
        if self.training and self.dropout_p > 0:
            mask = F.dropout(mask, p=0.2, training=True)
        H_filtered = H * mask
        g = IFFT(H_filtered)
...
class SpectralDropoutLowCfCCell(nn.Module):
    """CfC cell with r200 spectral gating + frequency dropout p=0.2."""
...
class SpectralDropoutLowCfCStackedNetwork(nn.Module):
    """Stacked SpectralDropoutLow-CfC with Kh ladder (p=0.2)."""
...
def make_lbps_lnkhlfft_specdroplow_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + Spectral Gating + Frequency Dropout p=0.2."""
    return SpectralDropoutLowCfCStackedNetwork(
...
__all__ = [
    "SpectralDropoutLowCfCCell",
    "SpectralDropoutLowCfCStackedNetwork",
