"""LearnedBetaPS+LN+Khl+FFT+5ScaleSpectralBiasDropout-CfC (PRD #10-179, Round 217, 2026-06-16).

Combines r216 (4-scale + bias + dropout) + adds a 5th scale
(sixteenth, very low frequencies). Pushes the scale count
beyond r212-r216.

5 scales: full (9) / half (5) / quarter (3) / eighth (2) /
sixteenth (1) frequencies for hidden=16.

Mechanism (same as r216 but with 5 scales):
    For each timestep t:
        z = cat([aug_x, aug_h]) → LN
        f, h_branch from z
        H1..H5 = FFT(h_t) at 5 scales
        For each scale:
            mag = |H| + bias
...
class FiveScaleSpectralBiasDropCfCCell(nn.Module):
    """CfC cell with 5-scale spectral gating + per-frequency bias + dropout."""
    ...
    def __init__(
        ...
        n_freq_sixteenth = max(hidden_size // 32 + 1, 1)
        ...
        self.spec_mask5 = nn.Linear(n_freq_sixteenth, n_freq_sixteenth)
        ...
        self.spec_bias5 = nn.Parameter(torch.zeros(n_freq_sixteenth))
    ...
    def _5scale_spectral(self, h):
        ...
        # Scale 5: sixteenth
        H5 = H1[:, :self.hidden_size // 32 + 1]
        mag5 = torch.abs(H5) + self.spec_bias5
        mask5 = torch.sigmoid(self.spec_mask5(mag5))
        if self.training and self.dropout_p > 0:
            mask5 = F.dropout(mask5, p=self.dropout_p, training=True)
        H5_full = torch.zeros_like(H1)
        H5_full[:, :H5.shape[1]] = H5 * mask5
        g5 = torch.fft.irfft(H5_full, n=self.hidden_size, dim=-1)
...
class FiveScaleSpectralBiasDropCfCStackedNetwork(nn.Module):
    """Stacked 5-scale spectral bias dropout CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_5spectralbiasdrop_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 5-Scale Spectral Gating + per-frequency bias + dropout p=0.2."""
    return FiveScaleSpectralBiasDropCfCStackedNetwork(
...
__all__ = [
    "FiveScaleSpectralBiasDropCfCCell",
    "FiveScaleSpectralBiasDropCfCStackedNetwork",
