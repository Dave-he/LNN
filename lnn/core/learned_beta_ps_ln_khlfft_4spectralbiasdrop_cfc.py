"""LearnedBetaPS+LN+Khl+FFT+4ScaleSpectralBiasDropout-CfC (PRD #10-178, Round 216, 2026-06-16).

Combines r212 (4-scale), r213 (dropout), r215 (bias) — all three
mechanisms together. 4 scales (full, half, quarter, eighth) +
per-frequency bias + dropout p=0.2 on each mask.
...
Mechanism:
    For each timestep t:
        ...
        # 4-scale spectral gating WITH bias + dropout
        H1 = FFT(h_t)  # full
        mag1 = |H1| + bias1
...
class FourScaleSpectralBiasDropCfCCell(nn.Module):
    """CfC cell with 4-scale spectral gating + per-frequency bias + dropout."""
...
class FourScaleSpectralBiasDropCfCStackedNetwork(nn.Module):
    """Stacked 4-scale spectral bias dropout CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_4spectralbiasdrop_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-Scale Spectral Gating + per-frequency bias + dropout p=0.2."""
