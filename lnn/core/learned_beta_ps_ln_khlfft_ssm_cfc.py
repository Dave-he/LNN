"""LearnedBetaPS+LN+Khl+FFT+SSM-CfC (PRD #10-169, Round 207, 2026-06-16).

Adds a **diagonal state-space model (SSM)** component to CfC
in the style of Mamba/S4. The SSM is a linear recurrence:
    h_ssm_t = A * h_ssm_{t-1} + B * x_t
    g_ssm = C * h_ssm_t
This provides a complementary gating signal to the CfC's
nonlinear branch.
...
Mechanism:
    For each timestep t:
        ...
        B = linear_B(z)                   # [B, H]
        C = linear_C(z)                   # [B, H]
        h_ssm = A * h_ssm + B * x_t       # [B, H]
        g_ssm = C * h_ssm                 # [B, H]
        # Combine SSM with branch
...
class SSMCfCCell(nn.Module):
    """CfC cell with diagonal SSM gating component."""
    ...
    def __init__(
        ...
        # SSM components: A (decay), B (input-to-state), C (state-to-output)
        self.ssm_A = nn.Sequential(nn.Linear(aug_total, hidden_size), nn.Softplus())
        # B projects from x_t (input_size) to hidden_size
    ...
    def _ssm_step(self, x_t, h_ssm, z):
        """One diagonal SSM step."""
        A = self.ssm_A(z)                                # [B, H]
        B = self.ssm_B(x_t)                              # [B, H], projects x_t
        C = self.ssm_C(z)                                # [B, H]
...
class SSMCfCStackedNetwork(nn.Module):
    """Stacked SSM-CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_ssm_5_3_2(input_size, hidden_size, output_size, num_layers=3):
    """Kh=[5,3,2] + LN + FFT + Diagonal SSM."""
    return SSMCfCStackedNetwork(
