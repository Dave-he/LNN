...
Mechanism:
    For each timestep t:
        ...
        # 4-scale spectral gating WITH dropout
        H1 = FFT(h_t)  # full
        mag1 = |H1|
        ...
        if training: mask4 = F.dropout(mask4, p=0.2)
        g4 = IFFT(H4_full * mask4, n=hidden_size)
...
class FourScaleSpectralDropCfCCell(nn.Module):
    """CfC cell with 4-scale spectral gating + dropout."""
...
class FourScaleSpectralDropCfCStackedNetwork(nn.Module):
    """Stacked 4-scale spectral dropout CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_4spectraldrop_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-Scale Spectral Gating + Dropout p=0.2."""
    return FourScaleSpectralDropCfCStackedNetwork(
...
__all__ = [
    "FourScaleSpectralDropCfCCell",
    "FourScaleSpectralDropCfCStackedNetwork",
