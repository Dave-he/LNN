"""LearnedBetaPS+LN+Khl+FFT+3ScaleAdaptiveSpectral-CfC (PRD #10-173, Round 211, 2026-06-16).

3-scale spectral with **learned per-scale weights** (adaptive
weighting). r210 used simple average; r211 uses softmax(linear(z))
weights.
...
class ThreeScaleAdaptiveSpectralCfCCell(nn.Module):
    """CfC cell with 3-scale adaptive spectral gating."""
    ...
    def __init__(
        ...
        n_freq_quarter = (hidden_size // 4) // 2 + 1
        ...
        # Adaptive weights
        self.scale_weight = nn.Linear(aug_total, 3)
    ...
    def _3scale_spectral(self, h, z):
        ...
        # Adaptive weights (NEW vs r210)
        w = torch.softmax(self.scale_weight(z), dim=-1)  # [B, 3]
        g_combined = w[:, 0:1] * g1 + w[:, 1:2] * g2 + w[:, 2:3] * g3
        return g_combined, w
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        g, _ = self._3scale_spectral(h_t, z)
...
class ThreeScaleAdaptiveSpectralCfCStackedNetwork(nn.Module):
    """Stacked 3-scale adaptive spectral CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_3spectraladapt_5_3_2(input_size, hidden_size, output_size, num_layers=3):
    """Kh=[5,3,2] + LN + FFT + 3-Scale Adaptive Spectral Gating (learned weights)."""
    return ThreeScaleAdaptiveSpectralCfCStackedNetwork(
...
__all__ = [
    "ThreeScaleAdaptiveSpectralCfCCell",
    "ThreeScaleAdaptiveSpectralCfCStackedNetwork",
