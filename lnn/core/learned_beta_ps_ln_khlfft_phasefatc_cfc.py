"""LearnedBetaPS+LN+Khl+FFT+PhaseFATC-CfC (PRD #10-188, Round 226, 2026-06-17).

Phase-Aware FATC: extends r225 by adding FFT phase (angle) to
the time-constant adaptation signal. Phase carries timing info,
magnitude carries strength. Combining both gives richer per-
feature time_scale.

Tests if phase awareness improves over magnitude-only FATC.
"""
...
class PhaseFATCSpectralCfCCell(nn.Module):
    """CfC cell with phase-aware frequency-adaptive time constants + 4-scale spectral."""
    ...
    def __init__(
        ...
        # PhaseFATC: phase + magnitude -> per-feature scale factor
        # Phase is in [-pi, pi], so we use 2 features per freq (cos, sin)
        n_freq = hidden_size // 2 + 1
        # Use raw phase (cos, sin) and magnitude as 3 features per freq
        self.freq_to_scale = nn.Linear(n_freq * 3, hidden_size)
    ...
    def _adaptive_time_scale(self, h):
        """Per-feature time_scale modulated by FFT magnitude + phase of h."""
        H = torch.fft.rfft(h, dim=-1)  # (B, n_freq)
        mag = torch.abs(H)  # (B, n_freq)
        # Phase as (cos, sin) - rotation-invariant
        cos_phase = torch.cos(torch.angle(H))  # (B, n_freq)
        sin_phase = torch.sin(torch.angle(H))  # (B, n_freq)
        # Concatenate: (B, n_freq * 3)
        feat = torch.cat([mag, cos_phase, sin_phase], dim=-1)
...
class PhaseFATCSpectralCfCStackedNetwork(nn.Module):
    """Stacked PhaseFATC + 4-scale spectral CfC."""
...
def make_lbps_lnkhlfft_phasefatc_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-scale spectral + PhaseFATC (mag+phase) + bias + dropout p=0.2."""
    return PhaseFATCSpectralCfCStackedNetwork(
