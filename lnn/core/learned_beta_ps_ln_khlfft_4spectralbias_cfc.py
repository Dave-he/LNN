"""LearnedBetaPS+LN+Khl+FFT+4ScaleSpectralBias-CfC (PRD #10-177, Round 215, 2026-06-16).

4-scale spectral gating with per-frequency learnable bias.
Each scale has a learnable bias added to the magnitude before
the mask linear, allowing the model to up/down-weight specific
frequencies.
...
Mechanism:
    For each timestep t:
        ...
        # 4-scale spectral gating WITH bias
        H1 = FFT(h_t)  # full
        mag1 = |H1| + bias1  # NEW
...
class FourScaleSpectralBiasCfCCell(nn.Module):
    """CfC cell with 4-scale spectral gating + per-frequency bias."""
    ...
    def __init__(
        ...
        # NEW: per-frequency learnable bias
...
class FourScaleSpectralBiasCfCStackedNetwork(nn.Module):
    """Stacked 4-scale spectral bias CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_4spectralbias_5_3_2(input_size, hidden_size, output_size, num_layers=3):
    """Kh=[5,3,2] + LN + FFT + 4-Scale Spectral Gating + per-frequency bias."""
    return FourScaleSpectralBiasCfCStackedNetwork(
...
__all__ = [
    "FourScaleSpectralBiasCfCCell",
    "FourScaleSpectralBiasCfCStackedNetwork",
