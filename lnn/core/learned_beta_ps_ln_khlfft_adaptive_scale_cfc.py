"""LearnedBetaPS+LN+Khl+FFT+AdaptiveScale-CfC (PRD #10-186, Round 224, 2026-06-17).

Single-cell adaptive scale selector: each cell computes 3
spectral branches (2-scale, 3-scale, 4-scale) and uses a
learned per-step gate to weight them.

Tests if adaptive scale selection (per-step) beats fixed
per-layer allocation (r223).
"""
...
class AdaptiveScaleSpectralBiasDropCfCCell(nn.Module):
    """CfC cell with adaptive 3-branch spectral gating (2/3/4 scales)."""
    ...
    def __init__(
        ...
        # 3 scale branches (2/3/4 scales each)
        # Branch 1: 2 scales (full, half)
        n_freq_full = hidden_size // 2 + 1
        n_freq_half = hidden_size // 4 + 1
        n_freq_quarter = hidden_size // 8 + 1
        n_freq_eighth = hidden_size // 16 + 1
        # Branch A: 2-scale
        self.spec_mask1_a = nn.Linear(n_freq_full, n_freq_full)
        self.spec_mask2_a = nn.Linear(n_freq_half, n_freq_half)
        self.spec_bias1_a = nn.Parameter(torch.zeros(n_freq_full))
        self.spec_bias2_a = nn.Parameter(torch.zeros(n_freq_half))
        # Branch B: 3-scale
        self.spec_mask1_b = nn.Linear(n_freq_full, n_freq_full)
        self.spec_mask2_b = nn.Linear(n_freq_half, n_freq_half)
        self.spec_mask3_b = nn.Linear(n_freq_quarter, n_freq_quarter)
        self.spec_bias1_b = nn.Parameter(torch.zeros(n_freq_full))
        self.spec_bias2_b = nn.Parameter(torch.zeros(n_freq_half))
        self.spec_bias3_b = nn.Parameter(torch.zeros(n_freq_quarter))
        # Branch C: 4-scale
        self.spec_mask1_c = nn.Linear(n_freq_full, n_freq_full)
        self.spec_mask2_c = nn.Linear(n_freq_half, n_freq_half)
        self.spec_mask3_c = nn.Linear(n_freq_quarter, n_freq_quarter)
        self.spec_mask4_c = nn.Linear(n_freq_eighth, n_freq_eighth)
        self.spec_bias1_c = nn.Parameter(torch.zeros(n_freq_full))
        self.spec_bias2_c = nn.Parameter(torch.zeros(n_freq_half))
        self.spec_bias3_c = nn.Parameter(torch.zeros(n_freq_quarter))
        self.spec_bias4_c = nn.Parameter(torch.zeros(n_freq_eighth))

        # Gate: input -> softmax over 3 branches
        self.scale_router = nn.Linear(aug_total, 3)
    ...
    def _branch_a_2scale(self, h):
        """2-scale branch: full + half."""
        H1 = torch.fft.rfft(h, dim=-1)
        mag1 = torch.abs(H1) + self.spec_bias1_a
        mask1 = torch.sigmoid(self.spec_mask1_a(mag1))
        ...
        mag2 = torch.abs(H2) + self.spec_bias2_a
        mask2 = torch.sigmoid(self.spec_mask2_a(mag2))
        if self.training and self.dropout_p > 0:
    ...
    def _branch_b_3scale(self, h):
        """3-scale branch: full + half + quarter."""
        H1 = torch.fft.rfft(h, dim=-1)
        mag1 = torch.abs(H1) + self.spec_bias1_b
        mask1 = torch.sigmoid(self.spec_mask1_b(mag1))
        if self.training and self.dropout_p > 0:
            mask1 = F.dropout(mask1, p=self.dropout_p, training=True)
        g1 = torch.fft.irfft(H1 * mask1, n=self.hidden_size, dim=-1)
        H2 = H1[:, :self.hidden_size // 4 + 1]
        mag2 = torch.abs(H2) + self.spec_bias2_b
        mask2 = torch.sigmoid(self.spec_mask2_b(mag2))
        if self.training and self.dropout_p > 0:
            mask2 = F.dropout(mask2, p=self.dropout_p, training=True)
        H2_full = torch.zeros_like(H1)
        H2_full[:, :H2.shape[1]] = H2 * mask2
        ...
        mag3 = torch.abs(H3) + self.spec_bias3_b
        mask3 = torch.sigmoid(self.spec_mask3_b(mag3))
        if self.training and self.dropout_p > 0:
    ...
    def _branch_c_4scale(self, h):
        """4-scale branch: full + half + quarter + eighth."""
        H1 = torch.fft.rfft(h, dim=-1)
        mag1 = torch.abs(H1) + self.spec_bias1_c
        mask1 = torch.sigmoid(self.spec_mask1_c(mag1))
        if self.training and self.dropout_p > 0:
            mask1 = F.dropout(mask1, p=self.dropout_p, training=True)
        g1 = torch.fft.irfft(H1 * mask1, n=self.hidden_size, dim=-1)
        H2 = H1[:, :self.hidden_size // 4 + 1]
        mag2 = torch.abs(H2) + self.spec_bias2_c
        mask2 = torch.sigmoid(self.spec_mask2_c(mag2))
        if self.training and self.dropout_p > 0:
            mask2 = F.dropout(mask2, p=self.dropout_p, training=True)
        H2_full = torch.zeros_like(H1)
        H2_full[:, :H2.shape[1]] = H2 * mask2
        g2 = torch.fft.irfft(H2_full, n=self.hidden_size, dim=-1)
        H3 = H1[:, :self.hidden_size // 8 + 1]
        mag3 = torch.abs(H3) + self.spec_bias3_c
        mask3 = torch.sigmoid(self.spec_mask3_c(mag3))
        if self.training and self.dropout_p > 0:
            mask3 = F.dropout(mask3, p=self.dropout_p, training=True)
        H3_full = torch.zeros_like(H1)
        H3_full[:, :H3.shape[1]] = H3 * mask3
        ...
        mag4 = torch.abs(H4) + self.spec_bias4_c
        mask4 = torch.sigmoid(self.spec_mask4_c(mag4))
        if self.training and self.dropout_p > 0:
    ...
    def _adaptive_spectral(self, h, z):
        """3-branch adaptive spectral with learned gate."""
        g_a = self._branch_a_2scale(h)
        g_b = self._branch_b_3scale(h)
        g_c = self._branch_c_4scale(h)
        # Gate from input z
        logits = self.scale_router(z)  # (B, 3)
        weights = F.softmax(logits, dim=-1)  # (B, 3)
        # Weighted combination
        g = weights[:, 0:1] * g_a + weights[:, 1:2] * g_b + weights[:, 2:3] * g_c
        return g, weights
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        g, weights = self._adaptive_spectral(h_t, z)
        ...
        return h_new, emas_x_new, emas_h_new, weights


class AdaptiveScaleSpectralCfCStackedNetwork(nn.Module):
    """Stacked adaptive-scale spectral CfC."""
    ...
    def __init__(
        ...
        for l in range(num_layers):
            ...
            self.cells.append(
                AdaptiveScaleSpectralBiasDropCfCCell(
    ...
    def forward(self, x):
        ...
        for t in range(T):
            ...
            for l, cell in enumerate(self.cells):
                hs[l], emas_x[l], emas_h[l], _weights = cell(
...
def make_lbps_lnkhlfft_adaptive_scale_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + adaptive scale (2/3/4 branch selector) + bias + dropout p=0.2."""
    return AdaptiveScaleSpectralCfCStackedNetwork(
