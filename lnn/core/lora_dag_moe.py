"""Round 123 — LoRA-DAG-MoE (Expert Family × Aggregation hybrid) for CfC.

Combines the two orthogonal winners of the 91-122 audit:
- **LoRA-MoRE (round 118)**: low-rank expert deltas (rank r) added to a
  shared base CfC, B-init-zero for warm start.
...
family** — but the audit showed routing and expert family are coupled
in 1D. This round combines **expert family + aggregation** — two
orthogonal mechanism dimensions — to test whether the combination
gives multiplicative gains.
...
Forward pass (L iterations of DAG refinement, K experts):
  ...
  g, top_idx = router(combined)              # top-K sparse or dense
  selected = gather(all_deltas, top_idx)     # [B, k, H]
  node_outs = g * selected + (1/k) * h_base  # init DAG nodes
  refined = dag(node_outs)                   # L iterations of DAGEdgeGate
  h_lora = sum_i refined[:, i, :]            # [B, H]
  h_new = h_base + h_lora

Notes
-----
* The LoRA experts use the same LoRAExpert from round 118 (B-init-zero
  warm start, scale = alpha/r).
* The DAG aggregation uses the same DAGEdgeGate from round 120
  (L iterations of pairwise gated projection, up-projection
  zero-initialized for early-training stability).
* The router is FAME (ForecastabilityRouter) by default for top-K
  sparse, or SigmoidRouter for dense (matching round 118).
* Parameter cost: K=3 rank=4 LoRA = ~3*8*4=96 + 3*4*16=192 = 288 LoRA
  params + 3 edge-gate parameters per DAG iteration (~few hundred
  per layer) + base CfC.
"""
...
# ---------------------------------------------------------------------------
# DAG Aggregation over LoRA deltas
# ---------------------------------------------------------------------------


class LoRADAGAggregation(nn.Module):
    """L iterations of DAG refinement over K LoRA-delta nodes.

    Same structure as ``DAGAggregation`` (round 120) — the input is
    the K selected LoRA deltas instead of K dense expert outputs.
    Each iteration applies a ``DAGEdgeGate`` with independent
    parameters.
    """
...
class LoRADAGMoECfCCell(nn.Module):
    """LoRA-DAG-MoE CfC cell.

    Combines LoRA experts (round 118) with DAG aggregation (round 120).
    """
    ...
    def __init__(
        ...
        # K LoRA adapters (B-init-zero warm start)
    ...
    def forward_with_aux(
        ...
        """Forward returning (h_new, info).

        info keys: all_deltas, g, top_idx, weighted, node_outs, refined, h_lora, h_base
        """
        B = x_t.size(0)
        ...
        if self.router_type == "learned":
            g = self.router(x_t, h)  # [B, n_experts] sparse (top_k nonzeros)
            top_idx = self.router.last_top_idx  # [B, top_k]
            top_k = self.top_k
            # Gather the top-K mixture weights from g
            ...
            # sigmoid router — may be sparse (top_k>0) or dense (top_k=0)
...
class LoRADAGMoECfCNetwork(nn.Module):
    """Stacked LoRA-DAG-MoE CfC network."""
...
def lora_dag_moe_utilization(cell: LoRADAGMoECfCCell) -> dict:
    """Diagnostic for LoRA-DAG-MoE cell's expert utilization."""
    n_total = sum(p.numel() for p in cell.parameters())
    n_dag = sum(p.numel() for p in cell.dag.parameters())
    n_lora = sum(p.numel() for e in cell.experts for p in e.parameters())
