"""LearnedBetaPS+LN+Khl+FFT+3ScaleSpectral-CfC (PRD #10-172, Round 210, 2026-06-16).

Three-scale spectral gating (Sonnet 2026 extended).
Apply r200's spectral gating at THREE resolutions (full, half,
quarter) and combine. Captures fine + medium + coarse spectral
structure.
...
class ThreeScaleSpectralCfCCell(nn.Module):
    """CfC cell with 3-scale spectral gating."""
...
class ThreeScaleSpectralCfCStackedNetwork(nn.Module):
    """Stacked 3-scale spectral CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_3spectral_5_3_2(input_size, hidden_size, output_size, num_layers=3):
    """Kh=[5,3,2] + LN + FFT + 3-Scale Spectral Gating."""
    return ThreeScaleSpectralCfCStackedNetwork(
...
__all__ = [
    "ThreeScaleSpectralCfCCell",
    "ThreeScaleSpectralCfCStackedNetwork",
