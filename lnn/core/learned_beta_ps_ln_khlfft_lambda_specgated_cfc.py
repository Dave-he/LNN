"""LearnedBetaPS+LN+Khl+FFT+LambdaSpecGated-CfC (PRD #10-164, Round 202, 2026-06-16).

Wraps round 187's winner (lbps_lnkhlfft_5_3_2) with
**learnable λ convex spectral gating** — adaptively mix
linear g_branch with spectral_g(h_t) based on input
complexity.

This is the follow-up to r200 (replace) and r201 (additive).
r200 was TD: sin -34.6% / struct 0% / random +12.2%.
r201 was NEG: sin -22.8% / struct 0% / random +11.2%.

r202 uses **learnable λ** to gate the spectral contribution:

  λ(z) = sigmoid(linear(z))           # ∈ [0, 1] per-timestep
  g_combined = (1-λ) * g_branch(z) + λ * spectral_g(h_t)
  h_new = tau_eff * g_combined + (1 - tau_eff) * h_branch

For periodic/clean data (sin): λ should learn → 1.0
(equivalent to r200 spec → -34.6% on sin).
For noisy data (random): λ should learn → 0.0
(equivalent to r187 baseline → preserves random).
This should give STRICTLY POSITIVE on all 3 datasets.
...
Mechanism:
    For each timestep t:
        ...
        # NEW (round 202): learnable λ convex combination
        λ = σ(linear(z))                  # ∈ [0, 1]
        H = FFT(h_t)
        magnitude = |H|
...
Hypothesis:
- H1 (positive): λ learns to use spectral on periodic
  data, linear on noisy → STRICTLY POSITIVE on all 3
- H2 (negative): λ doesn't learn meaningfully (always
  ~0.5 or collapses to one side)
- H3 (mixed): trade-off where combined is similar to
  baseline
"""
...
class LambdaSpecGatedCfCCell(nn.Module):
    """CfC cell with learnable λ convex combination of linear + spectral.

    g_combined = (1-λ(z)) * g_branch(z) + λ(z) * spectral_g(h_t)
    h_new = tau_eff * g_combined + (1 - tau_eff) * h_branch(z)

    The λ is content-aware (depends on z, which contains
    input + EMA features). This allows the model to adaptively
    use spectral on periodic data and linear on noisy data.
    """
    ...
    def __init__(
        ...
        # NEW (round 202): learnable λ + spectral gating on h.
        # λ: per-feature [B, H] from z (scalar projection per feature)
        self.lambda_gate = nn.Sequential(nn.Linear(aug_total, hidden_size), nn.Sigmoid())
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        # NEW (round 202): learnable λ convex combination.
        lam = self.lambda_gate(z)  # [B, H] in [0, 1]
        g_spec = self._spectral_gating(h_t)
        g_combined = (1.0 - lam) * g + lam * g_spec
...
class LambdaSpecGatedCfCStackedNetwork(nn.Module):
    """Stacked LambdaSpecGated-CfC with Kh ladder."""
...
def make_lbps_lnkhlfft_lambda_specgated_5_3_2(input_size, hidden_size, output_size, num_layers=3):
    """Kh=[5,3,2] + LN + FFT + Learnable λ Convex Spectral Gating."""
    return LambdaSpecGatedCfCStackedNetwork(
