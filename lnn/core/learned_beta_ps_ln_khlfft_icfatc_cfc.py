"""LearnedBetaPS+LN+Khl+FFT+IC-FATC-CfC (PRD #10-190, Round 228, 2026-06-17).

Input-Conditioned FATC (IC-FATC): extends r225 (FATC) by computing
the mix coefficient from the current INPUT x_t (not as a single
learned scalar per cell). This makes the FATC mechanism
input-aware — when the input is "spectrally rich", use more
FATC modulation; when input is simple, use base time_scale.

Pivot away from spectral axis (16 SPs r210-r227 saturated) to
input-axis mechanism class.
"""
...
class ICFATCSpectralCfCCell(nn.Module):
    """CfC cell with input-conditioned FATC + 4-scale spectral."""
    ...
    def __init__(
        ...
        # IC-FATC: mag -> per-feature scale factor (same as r225)
        n_freq = hidden_size // 2 + 1
        self.freq_to_scale = nn.Linear(n_freq, hidden_size)
        # IC mix: per-sample, per-feature mix from input
        self.ic_mix_proj = nn.Linear(input_size, hidden_size)
        self.ic_mix_bias = nn.Parameter(torch.tensor(0.0))
    ...
    def _adaptive_time_scale(self, h, x_t):
        """Per-feature time_scale modulated by FFT magnitude of h, with input-conditioned mix."""
        H = torch.fft.rfft(h, dim=-1)  # (B, n_freq)
        mag = torch.abs(H)  # (B, n_freq)
        scale_factor = torch.sigmoid(self.freq_to_scale(mag))  # (B, hidden)
        # IC mix: per-sample, per-feature, derived from x_t
        mix = torch.sigmoid(self.ic_mix_proj(x_t) + self.ic_mix_bias)  # (B, hidden)
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        time_scale = self._adaptive_time_scale(h_t, x_t)  # (B, hidden)
...
class ICFATCSpectralCfCStackedNetwork(nn.Module):
    """Stacked IC-FATC + 4-scale spectral CfC."""
...
def make_lbps_lnkhlfft_icfatc_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 4-scale spectral + IC-FATC (input-conditioned mix) + bias + dropout p=0.2."""
    return ICFATCSpectralCfCStackedNetwork(
