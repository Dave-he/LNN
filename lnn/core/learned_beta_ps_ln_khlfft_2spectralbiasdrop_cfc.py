"""LearnedBetaPS+LN+Khl+FFT+2ScaleSpectralBiasDropout-CfC (PRD #10-184, Round 222, 2026-06-16).

Minimal scale test. 2 scales: full / half. Combines r213+
r215+r216's mechanisms but with only 2 scales.

Tests if 2-scale + bias + dropout is the minimum viable scale.
"""
...
class TwoScaleSpectralBiasDropCfCCell(nn.Module):
    """CfC cell with 2-scale spectral + per-frequency bias + dropout."""
...
class TwoScaleSpectralBiasDropCfCStackedNetwork(nn.Module):
    """Stacked 2-scale spectral + bias + dropout CfC."""
...
def make_lbps_lnkhlfft_2spectralbiasdrop_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 2-Scale Spectral + per-frequency bias + dropout p=0.2."""
    return TwoScaleSpectralBiasDropCfCStackedNetwork(
...
__all__ = [
    "TwoScaleSpectralBiasDropCfCCell",
    "TwoScaleSpectralBiasDropCfCStackedNetwork",
