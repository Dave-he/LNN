"""LearnedBetaPS+LN+Khl+FFT+3ScaleSpectralBiasDropout-CfC (PRD #10-183, Round 221, 2026-06-16).

Combines r213 (3-scale + dropout) with r215 (per-frequency bias).
Tests if 3-scale + bias + dropout is competitive with r216 (4-scale
+ bias + dropout). 3 scales: full / half / quarter.

Mechanism:
    For each timestep t:
        z = cat([aug_x, aug_h]) → LN
        f, h_branch from z
        H1, H2, H3 = FFT(h_t) at 3 scales
        For each scale:
            mag = |H| + bias
...
class ThreeScaleSpectralBiasDropCfCCell(nn.Module):
    """CfC cell with 3-scale spectral + per-frequency bias + dropout."""
...
class ThreeScaleSpectralBiasDropCfCStackedNetwork(nn.Module):
    """Stacked 3-scale spectral + bias + dropout CfC."""
...
def make_lbps_lnkhlfft_3spectralbiasdrop_5_3_2(input_size, hidden_size, output_size, num_layers=3, dropout_p=0.2):
    """Kh=[5,3,2] + LN + FFT + 3-Scale Spectral + per-frequency bias + dropout p=0.2."""
    return ThreeScaleSpectralBiasDropCfCStackedNetwork(
...
__all__ = [
    "ThreeScaleSpectralBiasDropCfCCell",
    "ThreeScaleSpectralBiasDropCfCStackedNetwork",
