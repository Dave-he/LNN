"""LearnedBetaPS+LN-CfC (Per-Scale Learnable β + LayerNorm) (PRD #10-141, Round 179, 2026-06-16).

Variant of round 171's LearnedPerScaleBeta-CfC with **LayerNorm
applied to the combined augmented input** BEFORE the CfC linear
projections.

Hypothesis:
- H1 (positive): LayerNorm normalizes the augmented input
  (which has Kx*D + Kh*H features) → more stable training
- H2 (positive): LayerNorm helps with different scales in x
  (raw input vs EMAs with different β values)
- H3 (negative): LayerNorm destroys EMA scale information
...
Mechanism::
    ...
    For each timestep t:
        # Compute per-scale EMAs (round 171):
        ema_x_k = β_x_k * ema_x_k + (1 - β_x_k) * x_t
        ema_h_k = β_h_k * ema_h_k + (1 - β_h_k) * h_{t-1}
        # Build augmented input (concat or diff):
        aug_x = cat([x_t] + ema_x_k) or [x_t] + [ema_x_k - x_t]
        aug_h = cat([h_t] + ema_h_k) or [h_t] + [ema_h_k - h_t]
        z = cat([aug_x, aug_h])  # [B, (Kx+1)*D + (Kh+1)*H]
        # NEW (round 179): LayerNorm on z
        z = LayerNorm(z)
        # Closed-form CfC:
...
# ---------------------------------------------------------------------------
# Single cell with LayerNorm
# ---------------------------------------------------------------------------


class LearnedBetaPSLNCfCCell(nn.Module):
    """Single CfC cell with per-scale learned β AND LayerNorm."""
    ...
    def __init__(
        ...
        # LayerNorm on the combined augmented input.
    ...
    def forward(self, x_t, h_t, emas_x, emas_h, dt=1.0):
        ...
        # NEW (round 179): LayerNorm before CfC linear projections.
        z = self.layer_norm(z)
...
class LearnedBetaPSLNCfCStackedNetwork(nn.Module):
    """Stacked LearnedBetaPS+LN-CfC with LayerNorm on combined input."""
...
def make_lbps_ln_h3_75(input_size, hidden_size, output_size, num_layers=3):
    """3-layer, Kx=5, Kh=3, β_init=0.75, LayerNorm."""
    return LearnedBetaPSLNCfCStackedNetwork(
...
def make_lbps_ln_h2_75(input_size, hidden_size, output_size, num_layers=3):
    """3-layer, Kx=5, Kh=2, β_init=0.75, LayerNorm."""
    return LearnedBetaPSLNCfCStackedNetwork(
        input_size=input_size, hidden_size=hidden_size, output_size=output_size,
...
def make_lbps_ln_h5_75(input_size, hidden_size, output_size, num_layers=3):
    """3-layer, Kx=5, Kh=5, β_init=0.75, LayerNorm."""
    return LearnedBetaPSLNCfCStackedNetwork(
        input_size=input_size, hidden_size=hidden_size, output_size=output_size,
