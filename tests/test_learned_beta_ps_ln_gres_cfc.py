"""Round 185 — tests for LearnedBetaPS+LN+GatedResidual-CfC (PRD #10-147)."""
...
from lnn.core.learned_beta_ps_ln_gres_cfc import (
...
def test_cell_init():
    cell = LearnedBetaPSLNGatedResidualCfCCell(input_size=2, hidden_size=8, Kx=5, Kh=3)
    assert cell.Kx == 5
    assert cell.Kh == 3
    assert cell.layer_norm is not None
    assert cell.residual_proj is not None
    assert cell.alpha_raw is not None
    # Residual starts at 0 output
    assert torch.allclose(cell.residual_proj.bias, torch.zeros(8), atol=1e-6)
    # alpha starts at sigmoid(logit(0.1)) ≈ 0.1
    assert abs(cell.alpha.item() - 0.1) < 0.01
...
def test_cfc_step_unchanged_at_init():
    """At init, alpha ≈ 0.1 and residual is 0, so h_new ≈ h_cfc."""
    torch.manual_seed(0)
    cell = LearnedBetaPSLNGatedResidualCfCCell(input_size=2, hidden_size=8, Kx=5, Kh=3)
    x_t = torch.randn(2, 2)
    h_t = torch.zeros(2, 8)
    ...
    h_new, _, _ = cell(x_t, h_t, emas_x, emas_h)
    # At init: h_cfc should be the dominant term, gated residual adds 0
    # (residual is 0 since weight and bias are 0)
    assert h_new.abs().max().item() < 1.0  # reasonable scale
    assert torch.isfinite(h_new).all()


def test_alpha_is_learnable():
    """Alpha raw parameter is registered and receives gradient (may be 0 at init
    because residual_proj is zero-initialized, but should be non-None)."""
    torch.manual_seed(0)
    cell = LearnedBetaPSLNGatedResidualCfCCell(input_size=2, hidden_size=8, Kx=5, Kh=3)
    h_t = torch.randn(4, 8) * 0.1
    x_t = torch.randn(4, 2)
    emas_x = [torch.zeros(4, 2) for _ in range(5)]
    emas_h = [torch.zeros(4, 8) for _ in range(3)]
    h_new, _, _ = cell(x_t, h_t, emas_x, emas_h)
    target = torch.randn(4, 8)
    loss = F.mse_loss(h_new, target)
    loss.backward()
    # alpha_raw.grad may be 0 (h_residual=0 at init) but must not be None
    assert cell.alpha_raw.grad is not None
    # After training the residual becomes non-zero, alpha receives grad.
    with torch.no_grad():
        cell.residual_proj.weight.data = torch.randn_like(cell.residual_proj.weight) * 0.1
    h_new2, _, _ = cell(x_t, h_t, emas_x, emas_h)
    target2 = torch.randn(4, 8)
    loss2 = F.mse_loss(h_new2, target2)
    loss2.backward()
    assert cell.alpha_raw.grad is not None


def test_residual_is_learnable():
    """Residual can change after backward."""
    torch.manual_seed(0)
    cell = LearnedBetaPSLNGatedResidualCfCCell(input_size=2, hidden_size=8, Kx=5, Kh=3)
    x_t = torch.randn(4, 2)
    h_t = torch.zeros(4, 8)
...
def test_factory_h3_75():
    net = make_lbps_lngr_h3_75(input_size=2, hidden_size=8, output_size=1)
    assert net.Kx == 5
    assert net.Kh_ladder == [3, 3, 3]


def test_factory_h2_75():
    net = make_lbps_lngr_h2_75(input_size=2, hidden_size=8, output_size=1)
    assert net.Kh_ladder == [2, 2, 2]


def test_factory_h5_75():
    net = make_lbps_lngr_h5_75(input_size=2, hidden_size=8, output_size=1)
...
def test_gradient_flows_all_layers():
    """After a few training steps, gradient flows to all params."""
    torch.manual_seed(0)
    net = make_lbps_lngr_h3_75(input_size=2, hidden_size=8, output_size=1)
    x = torch.randn(2, 16, 2)
...
def test_no_sequences():
    net = LearnedBetaPSLNGatedResidualCfCStackedNetwork(
...
def test_smoke_learns_sin():
    torch.manual_seed(0)
    net = make_lbps_lngr_h3_75(input_size=2, hidden_size=12, output_size=1)
