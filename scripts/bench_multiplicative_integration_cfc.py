"""Round 142 — Bench Multiplicative Integration CfC on 3 datasets.

Tests whether multiplicative integration (Wu et al. 2016 NIPS) helps
our 1D time-series modelling.

Cells: 3 conditions × 3 datasets × 2 seeds = 18 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- mi_pure (pure multiplicative, x_proj ⊙ h_proj)
- mi_x_residual (multiplicative + additive x_proj residual)
"""
...
from lnn.core.multiplicative_integration_cfc import MultiplicativeIntegrationCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return MultiplicativeIntegrationCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, variant="pure", return_sequences=True,
        )
    elif cond == "mi_x_residual":
        return MultiplicativeIntegrationCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, variant="x_residual", return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_multiplicative_integration_cfc.json")
    ...
    print(f"=== Round 142 MI-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
