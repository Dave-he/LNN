"""Round 132 — Bench Antisymmetric CfC on 3 datasets.

Tests whether the AntisymmetricRNN constraint from arXiv:1811.02243 /
ICLR 2019 (Chang, Chen, Haber, Chu, "AntisymmetricRNN: A Dynamical
System View on Recurrent Neural Networks") helps our 1D time-series
modelling.
...
- antisym_small (Antisymm with init_scale=0.1)
- antisym_large (Antisymm with init_scale=0.5)
"""
...
from lnn.core.antisymmetric_cfc import AntisymmetricCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "antisym_small":
        return AntisymmetricCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, init_scale=0.1, return_sequences=True,
        )
    elif cond == "antisym_large":
        return AntisymmetricCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, init_scale=0.5, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_antisymmetric_cfc.json")
    ...
    print(f"=== Round 132 Antisymm bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
