"""Round 158 — bench for MultiBeta-CfC (Multi-Scale EMA Augmentation) (PRD #10-120).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- mb_diff_2 (β ∈ {0.7, 0.95}, diff mode)
- mb_concat_2 (β ∈ {0.7, 0.95}, concat mode)
- mb_diff_3 (β ∈ {0.5, 0.9, 0.99}, diff mode)
- mb_concat_3 (β ∈ {0.5, 0.9, 0.99}, concat mode)
...
from lnn.core.multi_beta_cfc import MultiBetaCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_learned_beta_cfc.py)
# ---------------------------------------------------------------------------
...
def make_mb_diff_2(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_mb_concat_2(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mb_diff_3(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mb_concat_3(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "mb_diff_2": make_mb_diff_2,
    "mb_concat_2": make_mb_concat_2,
    "mb_diff_3": make_mb_diff_3,
    "mb_concat_3": make_mb_concat_3,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_multi_beta_cfc.json")
    ...
    print(f"=== Round 158 MultiBeta-CfC Bench ===")
