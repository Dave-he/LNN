"""Round 156 — bench for EMA-X-CfC (Input EMA Augmentation) (PRD #10-118).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- ema_concat ([x, ema], 2D input)
- ema_gate (α·x + (1-α)·ema)
- ema_diff ([x, ema - x], 2D input)
- ema_ema_only (ema only, control)
...
from lnn.core.ema_x_cfc import EMAXCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_delta_cfc.py)
# ---------------------------------------------------------------------------
...
def make_ema_concat(input_size, hidden_size, output_size, num_layers=2):
    return EMAXCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_ema_gate(input_size, hidden_size, output_size, num_layers=2):
    return EMAXCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_ema_diff(input_size, hidden_size, output_size, num_layers=2):
    return EMAXCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_ema_ema_only(input_size, hidden_size, output_size, num_layers=2):
    return EMAXCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "ema_concat": make_ema_concat,
    "ema_gate": make_ema_gate,
    "ema_diff": make_ema_diff,
    "ema_ema_only": make_ema_ema_only,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_ema_x_cfc.json")
    ...
    print(f"=== Round 156 EMA-X-CfC Bench ===")
