"""Round 227 — bench for PhaseDriftSpectralCfC (PRD #10-189, 2026-06-17)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_phasedrift_cfc import make_lbps_lnkhlfft_phasedrift_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_fatc_5_3_2(D, hidden, 1)
    elif cond == "phasefatc":
        model = make_lbps_lnkhlfft_phasefatc_5_3_2(D, hidden, 1)
    elif cond == "phasedrift":
        model = make_lbps_lnkhlfft_phasedrift_5_3_2(D, hidden, 1)
    else:
...
def main():
    conds = ["cf", "4spectralbiasdrop", "fatc", "phasefatc", "phasedrift"]
    seeds = [0, 1]
    rows = []
    ...
    out = {
        "round": 227, "date": "2026-06-17",
        "config": "lbps_lnkhlfft_phasedrift_5_3_2 (mag+|H_diff| drift FATC) vs r225 (FATC) vs r226 (PhaseFATC) vs r216 vs cf",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_phasedrift_cfc.json"
