"""Round 203 — SpectralDropout-CfC bench (PRD #10-165, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_specgated_cfc import make_lbps_lnkhlfft_specgated_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_specgated_5_3_2(
            input_size=2, hidden_size=hidden, output_size=1,
        )
    elif cond == "specdrop":
    ...
    for ds_name in DATASETS:
        for cond in conds:
            for seed in [0, 1]:
                ...
                rows.append(row)
                print(f"  {ds_name:18s} {cond:8s} seed={seed} "
                      f"test_mse={row['test_mse']:.4f} time={row['elapsed_s']}s", flush=True)
    print("\n=== AGGREGATE (mean over seeds) ===", flush=True)
    print(f"{'cond':8s} {'sin_irr':10s} {'struct_irr':10s} {'random_irr':10s} {'mean':10s} {'Δ':6s}", flush=True)
    cf_mean = None
    for cond in conds:
        per_ds = {}
        for ds_name in DATASETS:
        ...
        print(f"{cond:8s} {per_ds['sin_irr']:.4f}     {per_ds['structured_irr']:.4f}     "
              f"{per_ds['random_irr']:.4f}     {m:.4f}     {delta}", flush=True)
    out = {
        "round": 203,
        "date": "2026-06-16",
        "config": "lbps_lnkhlfft_specdropout_5_3_2 (dropout_p=0.3) vs specgated_5_3_2 vs baseline_5_3_2",
        "epochs": 30,
        "seeds": [0, 1],
        "n_cells": len(rows),
        "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_specdropout_cfc.json"
