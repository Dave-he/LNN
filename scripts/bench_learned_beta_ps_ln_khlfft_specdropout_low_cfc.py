"""Round 205 — bench for SpectralDropoutLow-CfC (PRD #10-167, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_specdropout_low_cfc import make_lbps_lnkhlfft_specdroplow_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    elif cond == "specdrop_p0.3":
        model = make_lbps_lnkhlfft_specdropout_5_3_2(D, hidden, 1, dropout_p=0.3)
    elif cond == "specdrop_low_p0.2":
        model = make_lbps_lnkhlfft_specdroplow_5_3_2(D, hidden, 1, dropout_p=0.2)
    else:
...
def main():
    conds = ["cf", "specdrop_p0.3", "specdrop_low_p0.2"]
    seeds = [0, 1]
    rows = []
    ...
    out = {
        "round": 205, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_specdroplow_5_3_2 (p=0.2) vs specdropout (p=0.3) vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_specdropout_low_cfc.json"
