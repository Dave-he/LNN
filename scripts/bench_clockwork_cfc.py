"""Round 147 — Bench Clockwork CfC on 3 datasets.

Tests whether partitioning hidden into K modules (periods 1, 2, 4, ..., 2^{K-1})
...
- cw_k2 (K=2, periods 1,2)
- cw_k3 (K=3, periods 1,2,4)
- cw_k4 (K=4, periods 1,2,4,8)
"""
...
from lnn.core.clockwork_cfc import ClockworkCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return ClockworkCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, num_modules=2, return_sequences=True,
        )
    elif cond == "cw_k3":
        return ClockworkCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, num_modules=3, return_sequences=True,
        )
    elif cond == "cw_k4":
        return ClockworkCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, num_modules=4, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_clockwork_cfc.json")
    ...
    print(f"=== Round 147 Clockwork-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
