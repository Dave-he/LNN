"""Round 114 — ReMoE (ReLU-routed MoE) bench (PRD #10-76).

Compares ReMoE (K experts + ReLU router, fully differentiable) vs plain CfC
and FAME on 3 datasets.  Tests whether the cleaner gradient flow (ReLU
routing → gradient flows to all non-zero-gated experts) beats FAME (top-K
sparse, partial gradient) and matches DeepSeek (additive residual).
...
Conditions:
  ...
  - remoe_k3:       ReMoE K=3, no aux loss
  - remoe_k3_lb001: ReMoE K=3 + load balance aux loss λ=0.01
  - remoe_k3_lb01:  ReMoE K=3 + load balance aux loss λ=0.1
"""
...
from lnn.core.remoe import ReMoECfCNetwork, remoe_load_balancing_loss, remoe_utilization


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-113)
# ---------------------------------------------------------------------------
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond.startswith("remoe_"):
        return ReMoECfCNetwork(
            input_size=input_size,
            hidden_size=hidden_size,
            output_size=output_size,
            num_layers=2,
...
def collect_remoe_g(net) -> list:
    """Collect all per-step gates from a ReMoE network (across all layers, all steps)."""
    if not isinstance(net, ReMoECfCNetwork):
        return []
    # We need to re-run a forward pass to populate last_g on each cell.
    # The cell stashes the LAST step's gate, so for a multi-step sequence we
    # get the final timestep's gate.  That gives us a representative per-expert
    # gate distribution.
    with torch.no_grad():
        B, T, D = 4, 16, 2
        x_probe = torch.randn(B, T, D)
        _ = net(x_probe)
    gates = []
    for cell in net.cells:
        if isinstance(cell, ReMoECfCNetwork) or hasattr(cell, "last_g"):
            if cell.last_g is not None:
                gates.append(cell.last_g.cpu())
    return gates
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    aux_lambda = AUX_LOSS_LAMBDA[cond]
    ...
    for _epoch in range(epochs):
        ...
        if mask.sum() > 0:
            task_loss = F.mse_loss(out[mask], y_train_clean[mask])
        else:
            task_loss = F.mse_loss(out, y_train_clean)

        # ReMoE aux loss
        aux_loss = torch.tensor(0.0)
        if cond.startswith("remoe_") and aux_lambda > 0.0:
            gates = collect_remoe_g(net)
            if gates:
                # Use the last layer's gate (most representative)
                aux_loss = remoe_load_balancing_loss(gates[-1])

        loss = task_loss + aux_lambda * aux_loss
        loss.backward()
    ...
    # ReMoE utilization diagnostic
    sparsity = 0.0
    g_active_frac = []
    if cond.startswith("remoe_"):
        # Re-run a forward pass to populate last_g on each cell
        with torch.no_grad():
            _ = net(X_train)
        for cell in net.cells:
            if hasattr(cell, "last_g") and cell.last_g is not None:
                diag = remoe_utilization(cell)
                sparsity = float(diag["sparsity"].item())
                g_active_frac = diag["g_active_frac"].tolist()
                break  # use first layer's diagnostic for simplicity
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_remoe.json")
    ...
    print(f"=== Round 114 ReMoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
    for dataset in cells:
        for cond in conds:
            for seed in range(seeds):
                ...
                spars = f"spars={r['sparsity']:.3f}" if r["sparsity"] > 0 else ""
                print(
                    f"  [{len(results)}/{total}] {dataset}/{cond}/s{seed} "
                    f"-> test_mse={r['test_mse']:.4f} loss={r['final_loss']:.4f} {spars} ({elapsed:.0f}s)"
                )
