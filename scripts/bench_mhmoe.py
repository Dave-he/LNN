"""Round 115 — Multi-Head MoE (MH-MoE) bench (PRD #10-77).

Compares MH-MoE (H sub-tokens × K experts, per-sub-token top-K routing) vs
plain CfC and FAME on 3 datasets.  Tests whether the multi-head sub-token
split fixes the FAME H=0 collapse (every sub-token gets its own expert, so
all K are exercised on average).

Cells: 3 datasets × 5 conditions × 2 seeds = 30 cells
Conditions:
  - baseline_cfc:   standard CfC, no MoE (control)
  - fame_k3_t1:     FAME K=3 top_k=1 (round 78 baseline)
  - mhmoe_k4_h2_t1: MH-MoE K=4 H=2 top_k=1
  - mhmoe_k4_h2_t2: MH-MoE K=4 H=2 top_k=2
  - mhmoe_k4_h4_t1: MH-MoE K=4 H=4 top_k=1
"""
...
from lnn.core.mhmoe import MHMoECfCNetwork, mhmoe_utilization


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-114)
# ---------------------------------------------------------------------------


def make_sin_irr(B: int, T: int, D: int = 4, missing_rate: float = 0.3) -> torch.Tensor:
...
def make_structured_irr(B: int, T: int, D: int = 4, missing_rate: float = 0.3) -> torch.Tensor:
    t = torch.linspace(0, 4 * math.pi, T).unsqueeze(0).unsqueeze(-1).expand(B, T, 1)
    x = torch.zeros(B, T, D)
    for i in range(D):
...
def make_random_irr(B: int, T: int, D: int = 4, missing_rate: float = 0.3) -> torch.Tensor:
...
def make_net(cond: str, input_size: int = 4, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond.startswith("mhmoe_"):
        # Parse n_experts, n_heads, top_k from cond name
        # e.g., mhmoe_k4_h2_t1 -> K=4, H=2, top_k=1
        parts = cond.split("_")
        k = int(parts[1][1:])
        h = int(parts[2][1:])
        t = int(parts[3][1:])
        return MHMoECfCNetwork(
            input_size=input_size,
            hidden_size=hidden_size,
            output_size=output_size,
            num_layers=2,
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    # MH-MoE utilization diagnostic
    routing_entropy = 0.0
    expert_util = []
    if cond.startswith("mhmoe_"):
        with torch.no_grad():
            _ = net(X_train)
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_mhmoe.json")
    ...
    print(f"=== Round 115 MH-MoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
        ...
        for cond in conds:
            for seed in range(seeds):
                ...
                print(
                    ...
                    f"-> test_mse={r['test_mse']:.4f} loss={r['final_loss']:.4f} {ent} ({elapsed:.0f}s)"
                )
