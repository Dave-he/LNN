"""Round 118 — LoRA MoE bench (PRD #10-80).

Compares MoRE-style LoRA experts (arXiv:2505.22694) against plain CfC,
FAME, and Sigmoid MoE on 3 datasets.  Tests whether low-rank expert
decomposition is competitive at <dense FAME parameter cost.

Cells: 3 datasets × 6 conditions × 2 seeds = 36 cells
Conditions:
  ...
  - lora_k3_r1_dense:   LoRA-MoRE K=3 rank=1 dense (sigmoid router, extreme low-rank)
  - lora_k3_r4_dense:   LoRA-MoRE K=3 rank=4 dense (sigmoid router, mid-rank)
  - lora_k3_r4_top1:    LoRA-MoRE K=3 rank=4 top_k=1 (FAME router, sparse)
"""
...
from lnn.core.lora_moe import LoRACfCNetwork, lora_moe_utilization
from lnn.core.sigmoid_moe import SigmoidMoECfCNetwork


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-117)
# ---------------------------------------------------------------------------
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    # LoRA-MoE utilization diagnostic
    routing_entropy = 0.0
    expert_util = []
    n_lora_params = 0
    rank = 0
    scaling = 0.0
    if cond.startswith("lora_"):
        with torch.no_grad():
            _ = net(X_train)
        for cell in net.cells:
            if hasattr(cell, "last_expert_util") and cell.last_expert_util is not None:
                diag = lora_moe_utilization(cell)
                routing_entropy = float(diag["routing_entropy"].item()) if hasattr(diag["routing_entropy"], "item") else float(diag["routing_entropy"])
                expert_util = diag["expert_util"].tolist() if hasattr(diag["expert_util"], "tolist") else diag["expert_util"]
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_lora_moe.json")
    ...
    print(f"=== Round 118 LoRA MoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
