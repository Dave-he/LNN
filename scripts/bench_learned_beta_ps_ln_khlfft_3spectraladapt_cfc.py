"""Round 211 — bench for 3ScaleAdaptiveSpectralCfC (PRD #10-173, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_3spectraladapt_cfc import make_lbps_lnkhlfft_3spectraladapt_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    elif cond == "3spectraladapt":
        model = make_lbps_lnkhlfft_3spectraladapt_5_3_2(D, hidden, 1)
    else:
...
def main():
    conds = ["cf", "3spectral", "3spectraladapt"]
    seeds = [0, 1]
    rows = []
    ...
    out = {
        "round": 211, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_3spectraladapt_5_3_2 (learned weights) vs 3spectral vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_3spectraladapt_cfc.json"
