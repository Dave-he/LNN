"""Round 130 — Bench MR-MoE + Dual Attention CfC on 3 datasets.

Tests whether the multi-rate + dual-attention hybrid from
arXiv:2606.12240 (Zong, Boker, Eldardiry 2026) helps our recurrent
1D TS modelling.
...
Conditions:
- cfc (baseline)
- mr_moe_k3_uniform (all τ=1.0, no dual attn)
- mr_moe_k3_multirate (τ={0.1,1,10}, no dual attn)
- mr_moe_k3_dualattn (τ={0.1,1,10} + feat+temporal attn)
"""
...
from lnn.core.mr_moe_dualattn_cfc import MRMoEDualAttnCfCNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "mr_moe_k3_uniform":
        # All τ=1.0, no dual attn (ablation: multi-rate contribution = 0)
        return MRMoEDualAttnCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
            n_experts=3, top_k=2, tau_inits=(1.0, 1.0, 1.0),
            use_dual_attention=False,
        )
    elif cond == "mr_moe_k3_multirate":
        # Multi-rate only, no dual attn
        return MRMoEDualAttnCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
            n_experts=3, top_k=2, tau_inits=(0.1, 1.0, 10.0),
            use_dual_attention=False,
        )
    elif cond == "mr_moe_k3_dualattn":
        # Full multi-rate + dual attn
        return MRMoEDualAttnCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
            n_experts=3, top_k=2, tau_inits=(0.1, 1.0, 10.0),
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 30,
    ...
    # CRITICAL: replace NaN in input with 0 (CfC propagates NaN).
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_mr_moe_dualattn_cfc.json")
    ...
    print(f"=== Round 130 MR-MoE+DualAttn bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
