"""Round 148 — Bench GRU-D / Time-Decay CfC on 3 datasets.

Tests whether learnable per-feature time decay (GRU-D / CT-RNN style)
helps our 1D time-series modelling.

Cells: 4 conditions × 3 datasets × 2 seeds = 24 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- grud_g0 (γ ≈ 0, no decay — control)
- grud_g005 (γ ≈ 0.05, light decay)
...
from lnn.core.grud_cfc import TimeDecayCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        # γ ≈ 0: effectively no decay. softplus(-10) ≈ 4.5e-5.
        return TimeDecayCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, gamma_init=-10.0, return_sequences=True,
        )
    elif cond == "grud_g005":
        # γ ≈ 0.05: light decay. softplus(-3) ≈ 0.049.
        return TimeDecayCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, gamma_init=-3.0, return_sequences=True,
        )
    elif cond == "grud_g05":
        # γ ≈ 0.5: heavy decay. softplus(-0.4) ≈ 0.51.
        return TimeDecayCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, gamma_init=-0.4, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_grud_cfc.json")
    ...
    print(f"=== Round 148 GRU-D CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
