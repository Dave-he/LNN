"""Round 164 — bench for HybridBeta-XH-Deep-CfC (3-layer stacked) (PRD #10-126).

24 cells: 4 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline, 2-layer)
- hb_xh_deep_h1:    3-layer, Kx=1, Kh=1, scalar β=0.9 on h
- hb_xh_deep_h2:    3-layer, Kx=2, Kh=2, scalar β ∈ {0.7, 0.95} on h
- hb_xh_deep_h2_3x: 3-layer, Kx=3, Kh=2, scalar β ∈ {0.7, 0.95} on h
- hb_xh_deep_best:  3-layer, Kx=3, Kh=2, scalar β ∈ {0.7, 0.95} on h, both diff
...
# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_hybrid_beta_xh_cfc.py)
# ---------------------------------------------------------------------------
...
def make_cfc(input_size, hidden_size, output_size, num_layers=3):
    """3-layer CfC baseline (match depth)."""
    return CfCNetwork(
...
def make_cfc_2layer(input_size, hidden_size, output_size, num_layers=2):
    """2-layer CfC baseline (for round 163 reference)."""
    return CfCNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc_2layer": make_cfc_2layer,  # 2-layer CfC baseline
    "cfc_3layer": make_cfc,  # 3-layer CfC baseline
    "hb_xh_deep_h1": make_hb_xh_deep_h1,
    "hb_xh_deep_h2": make_hb_xh_deep_h2,
    "hb_xh_deep_h2_3x": make_hb_xh_deep_h2_3x,
    "hb_xh_deep_best": make_hb_xh_deep_best,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_hybrid_beta_xh_deep_cfc.json")
    ...
    print(f"=== Round 164 HybridBeta-XH-Deep-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, lr={args.lr}")
    print(f"3-layer stacked variants of round 163 hybrid β (per-feature x + scalar h)")
    ...
    for cond_name, cond_fn in CONDS.items():
        ...
        for ds_name, ds_fn in DATASETS.items():
            ...
            for seed in range(args.seeds):
                ...
                torch.manual_seed(seed)
                if cond_name in ("cfc_2layer",):
                    model = cond_fn(input_size=D, hidden_size=H, output_size=1, num_layers=2)
                else:
