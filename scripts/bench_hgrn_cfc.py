"""Round 131 — Bench HGRN CfC on 3 datasets.

Tests whether the HGRN bounded-forget-gate mechanism from
arXiv:2404.18807 (Qi et al. NeurIPS 2023) helps our 1D time-series
modelling.

Cells: 4 conditions × 3 datasets × 2 seeds = 24 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- hgrn_free (no lower bound, alpha=0)
- hgrn_bounded (single learnable alpha, hierarchical=False)
- hgrn_hierarchical (per-layer monotonic alpha, alpha_max=0.5)
"""
...
from lnn.core.hgrn_cfc import HGRNCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        # Free gate (alpha=0) — HGRN-free baseline.
        return HGRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.0, hierarchical=False,
            return_sequences=True,
        )
    elif cond == "hgrn_bounded":
        # Single learnable alpha, no hierarchy (uniform across layers).
        return HGRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.2, hierarchical=False,
            return_sequences=True,
        )
    elif cond == "hgrn_hierarchical":
        # Per-layer monotonic alpha: l=0 → 0, l=L-1 → alpha_max.
        return HGRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.0, hierarchical=True, alpha_max=0.5,
            return_sequences=True,
        )
    else:
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 30,
    ...
    # CRITICAL: replace NaN in input with 0.
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_hgrn_cfc.json")
    ...
    print(f"=== Round 131 HGRN bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
    ...
    print(f"{'cond':<26} | " + " | ".join(f"{d:<14}" for d in cells))
    for cond in conds:
        row = [cond]
        for dataset in cells:
    ...
    for cond in conds:
        ...
        print(f"  {cond:<26} n_params={params[0]}")
