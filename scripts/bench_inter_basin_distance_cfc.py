#!/usr/bin/env python3
"""Benchmark for InterBasinDistanceCfCCell (round 257).

Pivots the 11-round arc (r246-256, all about aux gating) to a new
**basin geometry diversification** axis: explicit inter-basin
repulsion pushes basin centers apart.

Modes:
  * baseline
  * r248_per_branch            (no repulsion)
  * r249_input_geom            (input+geom gate, current best structured)
  * r252_lyap_aux              (constant λ = 0.1)
  * r256_anneal_linear         (TIME-axis closure)
  * r257_inter_basin_d1        ⭐ (NEW, d_min=1.0)
  * r257_inter_basin_d2        ⭐ (NEW, d_min=2.0)
  * r257_inter_basin_d05       ⭐ (NEW, d_min=0.5)

Hypotheses:
  H1: r257 increases basin diversity (higher H_per_branch).
  H2: r257 matches or beats r248 on structured (more diverse geometry).
  H3: r257 protects against r252 regression.
"""

from __future__ import annotations

import argparse
import json
import math
import sys
from pathlib import Path

import torch
import torch.nn as nn

ROOT = Path(__file__).resolve().parent.parent
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from lnn.core.cfc import CfCCell  # noqa: E402
from lnn.core.per_branch_multibasin_lyapunov_cfc import (  # noqa: E402
    PerBranchMultiBasinLyapunovCfCCell,
)
from lnn.core.input_geometry_gated_per_branch_cfc import (  # noqa: E402
    InputGeometryGatedPerBranchCfCCell,
)
from lnn.core.lyap_aux_per_branch_multibasin_cfc import (  # noqa: E402
    LyapAuxPerBranchMultiBasinLyapunovCfCCell,
)
from lnn.core.annealed_per_branch_aux_cfc import (  # noqa: E402
    AnnealedPerBranchMultiBasinLyapunovCfCCell,
)
from lnn.core.inter_basin_distance_cfc import (  # noqa: E402
    InterBasinDistanceCfCCell,
)

DEVICE = "cpu"


def make_dataset(name: str, T: int = 64, d_in: int = 1, seed: int = 0):
    g = torch.Generator().manual_seed(seed)
    t = torch.linspace(0, 4 * math.pi, T).unsqueeze(-1)
    if name == "toy_sin":
        x = torch.sin(t)
        y = torch.cos(t)
    elif name == "structured":
        x = torch.sin(t) + 0.3 * torch.sin(3 * t)
        y = torch.roll(x, shifts=1, dims=0)
    elif name == "random":
        x = torch.randn(T, d_in, generator=g)
        y = torch.roll(x, shifts=1, dims=0)
    else:
        raise ValueError(f"unknown dataset {name}")
    return x, y


class SeqModel(nn.Module):
    def __init__(self, d_in: int, d_h: int, d_out: int, mode: str):
        super().__init__()
        self.mode = mode
        self.d_h = d_h
        if mode == "baseline":
            self.cell = CfCCell(d_in, d_h)
        elif mode == "r248_per_branch":
            self.cell = PerBranchMultiBasinLyapunovCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
            )
        elif mode == "r249_input_geom":
            self.cell = InputGeometryGatedPerBranchCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
            )
        elif mode == "r252_lyap_aux":
            self.cell = LyapAuxPerBranchMultiBasinLyapunovCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
                lyap_lambda=0.1,
            )
        elif mode == "r256_anneal_linear":
            self.cell = AnnealedPerBranchMultiBasinLyapunovCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
                lyap_lambda_max=0.1, anneal_epochs=50,
                anneal_schedule="linear",
            )
        elif mode == "r257_inter_basin_d05":
            self.cell = InterBasinDistanceCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
                d_min=0.5,
            )
        elif mode == "r257_inter_basin_d1":
            self.cell = InterBasinDistanceCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
                d_min=1.0,
            )
        elif mode == "r257_inter_basin_d2":
            self.cell = InterBasinDistanceCfCCell(
                d_in, d_h, n_branches=4, n_basin=3,
                tau_min=0.05, tau_max=20.0, seed=42,
                d_min=2.0,
            )
        else:
            raise ValueError(f"unknown mode {mode}")
        self.head = nn.Linear(d_h, d_out)

    def forward(self, x: torch.Tensor
                 ) -> tuple[torch.Tensor, list[dict]]:
        B = x.shape[1]
        outs, aux_per_step = [], []
        if self.mode in ("r248_per_branch", "r249_input_geom",
                         "r252_lyap_aux", "r256_anneal_linear"):
            h_list = self.cell.init_state(B, device=x.device)
            for t in range(x.shape[0]):
                h, h_list, aux = self.cell.forward_with_aux(
                    x[t], h_list, lyap_lambda=0.0, sep_lambda=0.0,
                )
                outs.append(self.head(h))
                aux_per_step.append(aux)
        elif self.mode in ("r257_inter_basin_d05", "r257_inter_basin_d1",
                           "r257_inter_basin_d2"):
            h_list = self.cell.init_state(B, device=x.device)
            for t in range(x.shape[0]):
                h, h_list, aux = self.cell.forward_with_aux(
                    x[t], h_list, lyap_lambda=0.0, sep_lambda=0.0,
                    dist_lambda=1.0,
                )
                outs.append(self.head(h))
                aux_per_step.append(aux)
        else:
            h = torch.zeros(B, self.cell.hidden_size, device=x.device)
            for t in range(x.shape[0]):
                h = self.cell(x[t], h)
                outs.append(self.head(h))
                aux_per_step.append({})
        return torch.stack(outs), aux_per_step


def train_one(name: str, seed: int, mode: str, epochs: int = 100, d_h: int = 9):
    torch.manual_seed(seed)
    x, y = make_dataset(name, seed=seed)
    x = x.unsqueeze(1).to(DEVICE)
    y = y.unsqueeze(1).to(DEVICE)

    model = SeqModel(d_in=x.shape[-1], d_h=d_h, d_out=y.shape[-1],
                     mode=mode).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=1e-2)

    aux_modes_with_aux = (
        "r252_lyap_aux", "r256_anneal_linear",
        "r257_inter_basin_d05", "r257_inter_basin_d1",
        "r257_inter_basin_d2",
    )

    dist_first = 0.0
    dist_last = 0.0
    H_first = 0.0
    H_last = 0.0

    for ep in range(epochs):
        if mode.startswith("r256_"):
            model.cell.set_epoch(ep)
        opt.zero_grad()
        pred, aux = model(x)
        task = ((pred - y) ** 2).mean()
        loss = task
        if mode in aux_modes_with_aux:
            if mode.startswith("r257_"):
                # Inter-basin loss (geometric).
                ival = [a.get("inter_basin_loss", torch.tensor(0.0))
                        for a in aux]
                dist_loss = sum(ival) / max(len(ival), 1)
                loss = loss + dist_loss
                if ep == 0:
                    dist_first = dist_loss.item()
                    H_first = sum(
                        a.get("mean_basin_H", torch.tensor(0.0)).item()
                        for a in aux
                    ) / max(len(aux), 1)
                if ep == epochs - 1:
                    dist_last = dist_loss.item()
                    H_last = sum(
                        a.get("mean_basin_H", torch.tensor(0.0)).item()
                        for a in aux
                    ) / max(len(aux), 1)
            else:
                lyap_vals = [a.get("lyap_loss", torch.tensor(0.0))
                             for a in aux]
                aux_loss = sum(lyap_vals) / max(len(lyap_vals), 1)
                loss = loss + aux_loss
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()

    with torch.no_grad():
        pred, aux = model(x)
        test_mse = ((pred - y) ** 2).mean().item()

    return {
        "dataset": name,
        "seed": seed,
        "mode": mode,
        "test_mse": test_mse,
        "dist_first": dist_first,
        "dist_last": dist_last,
        "H_first": H_first,
        "H_last": H_last,
    }


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--datasets", nargs="+",
                        default=["toy_sin", "structured", "random"])
    parser.add_argument("--seeds", nargs="+", type=int, default=[0, 1, 2])
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--d_h", type=int, default=9)
    parser.add_argument("--modes", nargs="+",
                        default=["baseline", "r248_per_branch", "r249_input_geom",
                                 "r252_lyap_aux", "r256_anneal_linear",
                                 "r257_inter_basin_d05", "r257_inter_basin_d1",
                                 "r257_inter_basin_d2"])
    parser.add_argument("--out", type=str,
                        default="analysis/inter_basin_distance_cfc_bench.json")
    args = parser.parse_args()

    rows = []
    for mode in args.modes:
        for ds in args.datasets:
            for seed in args.seeds:
                r = train_one(ds, seed, mode,
                              epochs=args.epochs, d_h=args.d_h)
                rows.append(r)
                print(f"[{mode:>22s}] ds={ds:>9s} seed={seed} "
                      f"test_mse={r['test_mse']:.4f} "
                      f"dist_first={r['dist_first']:.3f} "
                      f"dist_last={r['dist_last']:.3f} "
                      f"H_first={r['H_first']:.3f} "
                      f"H_last={r['H_last']:.3f}")

    summary: dict = {"rows": rows, "verdict": {}}
    for ds in args.datasets:
        ds_rows = [r for r in rows if r["dataset"] == ds]
        if not ds_rows:
            continue
        means = {m: sum(r["test_mse"] for r in ds_rows
                        if r["mode"] == m) / max(
            len([r for r in ds_rows if r["mode"] == m]), 1)
                 for m in args.modes}
        H_means = {m: sum(r["H_last"] for r in ds_rows
                          if r["mode"] == m) / max(
            len([r for r in ds_rows if r["mode"] == m]), 1)
                   for m in args.modes}
        summary["verdict"][ds] = {
            **{f"{m}_mse": means[m] for m in args.modes},
            **{f"{m}_H": H_means[m] for m in args.modes},
        }

    print("\n=== Verdict (round 257 — InterBasinDistance) ===")
    mode_order = [
        "baseline", "r248_per_branch", "r249_input_geom",
        "r252_lyap_aux", "r256_anneal_linear",
        "r257_inter_basin_d05", "r257_inter_basin_d1", "r257_inter_basin_d2",
    ]
    for ds, v in summary["verdict"].items():
        mse_parts = []
        for m in mode_order:
            key = f"{m}_mse"
            if key in v:
                tag = m.replace("r257_inter_basin_", "r257_") if m.startswith("r257_") else m
                mse_parts.append(f"{tag}={v[key]:.4f}")
        print(f"[{ds:>9s}] " + " ".join(mse_parts))
        H_parts = []
        for m in mode_order:
            key = f"{m}_H"
            if key in v:
                tag = m.replace("r257_inter_basin_", "r257_") if m.startswith("r257_") else m
                H_parts.append(f"H_{tag}={v[key]:.3f}")
        print(f"           " + " ".join(H_parts))

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nSaved {out_path}")


if __name__ == "__main__":
    main()
