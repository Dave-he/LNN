"""Round 141 — Bench Adaptive Time-Constant CfC on 3 datasets.

Tests whether input-conditional time constants (Adaptive Computation
Time, Graves 2016) help our 1D time-series modelling.
...
from lnn.core.adaptive_time_constant_cfc import AdaptiveTimeConstantCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "atc_concat":
        return AdaptiveTimeConstantCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, mode="concat", return_sequences=True,
        )
    elif cond == "atc_input":
        return AdaptiveTimeConstantCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, mode="input", return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_adaptive_time_constant_cfc.json")
    ...
    print(f"=== Round 141 ATC-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
