"""Round 149 — Bench Temporal Conv Concat CfC (TCC-CfC) on 3 datasets.

Tests whether a parallel 1D temporal conv stream concatenated with x
...
- tcc_k3 (K=3 conv, 1-step lookback)
- tcc_k5 (K=5 conv, 2-step lookback)
- tcc_k7 (K=7 conv, 3-step lookback)
"""
...
from lnn.core.tcc_cfc import TemporalConvConcatCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return TemporalConvConcatCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, kernel_size=3, return_sequences=True,
        )
    elif cond == "tcc_k5":
        return TemporalConvConcatCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, kernel_size=5, return_sequences=True,
        )
    elif cond == "tcc_k7":
        return TemporalConvConcatCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, kernel_size=7, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_tcc_cfc.json")
    ...
    print(f"=== Round 149 TCC-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
