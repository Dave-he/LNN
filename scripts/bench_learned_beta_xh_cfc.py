"""Round 162 — bench for LearnedBeta-XH-CfC (Per-Feature β on Stacked XH) (PRD #10-124).
...
β_init=2.197 (sigmoid(2.197) ≈ 0.9).
"""
...
from lnn.core.learned_beta_xh_cfc import LearnedBetaXHCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_stacked_ema_xh_cfc.py)
# ---------------------------------------------------------------------------
...
def make_lb_xh_diff_1_1(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_lb_xh_diff_3_2(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_lb_xh_concat_2_2(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_lb_xh_best(input_size, hidden_size, output_size, num_layers=2):
    """Kx=3 diff + Kh=2 diff, per-feature learned β (best config)."""
    return LearnedBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "lb_xh_diff_1_1": make_lb_xh_diff_1_1,
    "lb_xh_diff_3_2": make_lb_xh_diff_3_2,
    "lb_xh_concat_2_2": make_lb_xh_concat_2_2,
    "lb_xh_best": make_lb_xh_best,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_learned_beta_xh_cfc.json")
    ...
    print(f"=== Round 162 LearnedBeta-XH-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, L={L}, lr={args.lr}")
    print(f"Per-feature learned β (Kx options: 1 | 2 | 3, Kh options: 1 | 2)")
    print(f"β_init=2.197 (sigmoid(2.197) ≈ 0.9)")
