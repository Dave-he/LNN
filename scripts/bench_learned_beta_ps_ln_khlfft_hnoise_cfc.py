"""Round 193 — bench for hidden state Gaussian noise augmentation (PRD #10-155)."""
...
def run_dataset(name, make_fn, n_seeds=2, n_epochs=30, hidden=12):
    ...
    conds = [
        ...
        ("lbps_lnkhlfft_hnoise_5_3_2_05", lambda: make_lbps_lnkhlfft_hnoise_5_3_2(2, hidden, 1, hnoise_sigma=0.05)),
        ("lbps_lnkhlfft_hnoise_5_3_2_10", lambda: make_lbps_lnkhlfft_hnoise_5_3_2(2, hidden, 1, hnoise_sigma=0.10)),
    ]
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "results", "bench_learned_beta_ps_ln_khlfft_hnoise_cfc.json")
    ...
    for ds, rows in out["datasets"].items():
        print(f"\n{ds}:")
        for cname in ["lbps_lnkhlfft_5_3_2_mse", "lbps_lnkhlfft_hnoise_5_3_2_05", "lbps_lnkhlfft_hnoise_5_3_2_10"]:
