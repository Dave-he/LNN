"""Round 191 — bench for Energy Distance Loss + CfC (PRD #10-153).

12 cells: 2 conds × 3 datasets × 2 seeds × 30 epochs:
- lbps_lnkhlfft_5_3_2_mse: round 187 winner, pure MSE loss (baseline)
- lbps_lnkhlfft_5_3_2_ed: same model, ED + MSE combined loss (γ=0.1)
...
from lnn.core.energy_distance_loss import combined_energy_loss
...
def train_and_eval(model, x_train, y_train, x_eval, y_eval, epochs, lr, loss_type, gamma=0.1):
    """Train with MSE or ED+MSE and eval with MSE."""
    ...
    for _ in range(epochs):
        ...
        if loss_type == "mse":
            ...
            loss = combined_energy_loss(y_train, out, gamma=gamma)
        else:
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_energy_distance_loss_cfc.json")
    ...
    print(f"=== Round 191 Energy Distance Loss Bench ===")
    ...
    CONDS = [
        ...
        ("lbps_lnkhlfft_5_3_2_ed", "ed", args.gamma),
    ]
