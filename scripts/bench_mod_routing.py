"""Round 111 — Mixture-of-Depths routing bench (PRD #10-73).

Compares MoD-style top-k routing vs standard CfC on 3 datasets,
measuring test_mse, routing entropy, and fraction of timesteps
actually processed (the "compute" budget).

Cells: 3 datasets × 4 conditions × 2 seeds = 24 cells
Conditions:
  - baseline_cfc: standard CfC network, no MoD (control)
  - mod_all:      MoD with cap_k = T (process all, same as baseline)
  - mod_half:     MoD with cap_k_frac = 0.5
  - mod_quarter:  MoD with cap_k_frac = 0.25
"""
...
from lnn.core.mod_routing import MoDCfCNetwork, compute_mod_aux_loss


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-110)
# ---------------------------------------------------------------------------
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "baseline_cfc":
        ...
        return MoDCfCNetwork(
            input_size=input_size,
            hidden_size=hidden_size,
            output_size=output_size,
            num_layers=2,
            return_sequences=True,
            cap_k=None,  # all timesteps processed
        )
    elif cond == "mod_half":
        return MoDCfCNetwork(
            input_size=input_size,
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    for _epoch in range(epochs):
        opt.zero_grad()
        out = net(X_train)
        # MoD networks accumulate aux_loss; baseline has none.
        if cond.startswith("mod_"):
    ...
    # Compute process fraction: how many timesteps were actually processed
    # by the MoD cells (averaged over layers and timesteps).
    process_frac = 1.0  # baseline always processes
    if cond.startswith("mod_") and cond != "mod_all":
        process_frac = (
            0.5 if cond == "mod_half" else (0.25 if cond == "mod_quarter" else 1.0)
        )
...
CONDITIONS = ["baseline_cfc", "mod_all", "mod_half", "mod_quarter"]
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_mod_routing.json")
    ...
    print(f"=== Round 111 MoD bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
        ...
        for cond in conds:
            for seed in range(seeds):
                ...
                print(
                    ...
                    f"process={r['process_frac']:.2f} ({elapsed:.0f}s)"
                )
    ...
    for r in results:
        ...
        if key not in summary:
            summary[key] = {"test_mse": [], "final_loss": [], "process_frac": []}
        summary[key]["test_mse"].append(r["test_mse"])
        summary[key]["final_loss"].append(r["final_loss"])
        summary[key]["process_frac"].append(r["process_frac"])
    ...
    for cond in conds:
        row = f"{cond:<14} | "
        for d in cells:
            vals = summary[(cond, d)]["test_mse"]
            row += f"{sum(vals)/len(vals):.4f}±{torch.tensor(vals).std().item():.4f}  | "
        print(row)
    ...
    with open(args.output, "w") as f:
        json.dump(
            {"results": results, "summary": {f"{k[0]}/{k[1]}": v for k, v in summary.items()}},
            f, indent=2,
        )
