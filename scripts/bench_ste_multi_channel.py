#!/usr/bin/env python3
"""Benchmark for STEWithEntropy multi-channel input (round 274).

Tests whether r267/r272 production settings (h=192, T=64, d_in=1)
hold for d_in=4 (multi-channel input with noise channels), or
whether the win is d_in-specific.

The hypothesis (PRD #10-111): d_in=4 may help (more info per
timestep) or hurt (noise dilution). The extra 3 channels are
small gaussian noise (std=0.01).

Modes (4 total):
  * ste_entropy_d1_h64     — r270 reference (d=1, h=64)
  * ste_entropy_d1_h192    — r272 PRODUCTION (d=1, h=192)
  * ste_entropy_d4_h64     — NEW (d=4, h=64)
  * ste_entropy_d4_h192    — NEW (d=4, h=192, full prod)

Hypotheses (PRD #10-111):
...
# ---------------------------------------------------------------------------
# Toy data generators (d_in parameterizes input channels)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mode definitions (d_in, hidden_size)
# ---------------------------------------------------------------------------
MODES = {
    "ste_entropy_d1_h64": dict(d_in=1, hidden_size=64, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d1_h192": dict(d_in=1, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d4_h64": dict(d_in=4, hidden_size=64, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d4_h192": dict(d_in=4, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
}


def make_model(mode_cfg: dict) -> nn.Module:
    enc = WrappedSTEWithEntropy(
        mode_cfg["d_in"], mode_cfg["hidden_size"],
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_multi_channel_bench.json"
    )
        ...
        for ds_name in args.datasets:
            for seed in args.seeds:
                ...
                x, y = DATA_FACTORIES[ds_name](
                    T=args.T, n_samples=args.n_samples, d_in=d_in, seed=seed
                )
                N = x.shape[0]
                split = int(0.8 * N)
                ...
                print(
                    f"[bench] mode={mode_name:24s} d_in={d_in} ds={ds_name:10s} "
