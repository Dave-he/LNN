"""Round 134 — Bench Gated Input Skip CfC on 3 datasets.

Tests whether the Highway-style gated skip from
arXiv:1505.00387 (Srivastava, Greff, Schmidhuber, 2015, "Highway
Networks") helps our 1D time-series modelling.

Cells: 4 conditions × 3 datasets × 2 seeds = 24 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- gis_weak (skip_init_scale=0.01, gate_init_bias=0.0) — tiny skip
- gis_strong (skip_init_scale=0.5, gate_init_bias=0.0) — strong skip
- gis_open (skip_init_scale=0.1, gate_init_bias=2.0) — open gate (~0.88)
"""
...
from lnn.core.gated_input_skip_cfc import GatedInputSkipCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return GatedInputSkipCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, skip_init_scale=0.01, gate_init_bias=0.0,
            return_sequences=True,
        )
    elif cond == "gis_strong":
        return GatedInputSkipCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, skip_init_scale=0.5, gate_init_bias=0.0,
            return_sequences=True,
        )
    elif cond == "gis_open":
        return GatedInputSkipCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, skip_init_scale=0.1, gate_init_bias=2.0,
            return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_gated_input_skip_cfc.json")
    ...
    print(f"=== Round 134 GatedInputSkip bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
