"""Round 150 — bench for Linear-Nonlinear CfC (LiNo-CfC) (PRD #10-112).

24 cells: 4 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- lino_sum (linear + CfC, summed — main)
- lino_concat (linear + CfC, concatenated — control)
- lino_lin_only (just linear, no CfC — sanity)
...
from lnn.core.lino_cfc import LinearNonlinearCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_tcc_cfc.py)
# ---------------------------------------------------------------------------
...
def make_random_irr(B=32, T=32, D=2, missing_rate=0.3, seed=0):
    ...
    y = full.clone()  # target is the true cumsum (no NaN)
    return x, y
...
def make_lino_sum(input_size, hidden_size, output_size, num_layers=2):
    return LinearNonlinearCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_lino_concat(input_size, hidden_size, output_size, num_layers=2):
    return LinearNonlinearCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
class LinearOnlyNetwork(torch.nn.Module):
    """Just a linear projection (no recurrence) — sanity baseline."""
...
def make_lino_lin_only(input_size, hidden_size, output_size, num_layers=2):
    return LinearOnlyNetwork(input_size, hidden_size, output_size, num_layers=num_layers)


CONDS = {
    "cfc": make_cfc,
    "lino_sum": make_lino_sum,
    "lino_concat": make_lino_concat,
    "lino_lin_only": make_lino_lin_only,
}
...
def train_and_eval(model, x_train, y_train, x_eval, y_eval, epochs, lr):
    """Train model and return final eval loss."""
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_lino_cfc.json")
    ...
    print(f"=== Round 150 LiNo-CfC Bench ===")
