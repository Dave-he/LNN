#!/usr/bin/env python3
"""Benchmark for STEWithEntropy longer sequences (round 273).

Tests whether r267/r272 production settings (h=192, τ=1.0, λ=0.1)
hold for T=128 sequences, or whether the win is T-specific.

The hypothesis (PRD #10-110): T=128 may help (more averaging)
or hurt (BPTT instability, τ drift).

Modes (4 total):
  * ste_entropy_t64_h64    — r270 reference (T=64, h=64)
  * ste_entropy_t64_h192   — r272 PRODUCTION (T=64, h=192)
  * ste_entropy_t128_h64   — NEW (T=128, h=64)
  * ste_entropy_t128_h192  — NEW (T=128, h=192, full prod)

Hypotheses (PRD #10-110):

  H1: T=128 ≥ T=64 on structured (longer helps)
  H2: h=192 still wins at T=128 (hidden effect independent of T)
...
# ---------------------------------------------------------------------------
# Toy data generators (T parameterizes sequence length)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mode definitions (T, hidden_size)
# ---------------------------------------------------------------------------
MODES = {
    "ste_entropy_t64_h64": dict(T=64, hidden_size=64, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_t64_h192": dict(T=64, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_t128_h64": dict(T=128, hidden_size=64, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_t128_h192": dict(T=128, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
}
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_longer_sequences_bench.json"
    )
        ...
        for ds_name in args.datasets:
            for seed in args.seeds:
                ...
                x, y = DATA_FACTORIES[ds_name](
                    T=T, n_samples=args.n_samples, d_in=args.d_in, seed=seed
                )
                N = x.shape[0]
                split = int(0.8 * N)
                ...
                print(
                    f"[bench] mode={mode_name:24s} T={T:3d} ds={ds_name:10s} "
