#!/usr/bin/env python3
"""Benchmark for STEWithEntropy density sweep (round 275).

Tests whether r267/r272 production settings (density=0.3, h=192,
T=64, d_in=1, λ=0.1) hold for different densities, or whether
density=0.3 is optimal.

The hypothesis (PRD #10-112): density controls how many neurons
participate per step. Too few → not enough capacity. Too many →
less STE benefit (more neurons, weaker sparsity signal).

Modes (4 total):
  * ste_entropy_d0.1_h192   — NEW (sparse)
  * ste_entropy_d0.3_h192   — r267 PRODUCTION (current)
  * ste_entropy_d0.5_h192   — NEW (dense)
  * ste_entropy_d0.7_h192   — NEW (very dense)

Hypotheses (PRD #10-112):
  ...
  H5: logit_std grows with density (more neurons share budget)
...
# ---------------------------------------------------------------------------
# Mode definitions (density sweep at h=192)
# ---------------------------------------------------------------------------
MODES = {
    "ste_entropy_d0.1_h192": dict(density=0.1, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d0.3_h192": dict(density=0.3, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d0.5_h192": dict(density=0.5, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_d0.7_h192": dict(density=0.7, hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
}
...
def collect_diagnostics(model: SeqModel) -> dict:
    ...
    n_active = int(round(cell.density * cell.hidden_size))
    return {
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_density_bench.json"
    )
