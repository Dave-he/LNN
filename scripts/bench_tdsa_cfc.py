"""Round 152 — bench for Time-Domain Self-Attention CfC (TDSA-CfC) (PRD #10-114).

24 cells: 4 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- tdsa (1 head, attn_dim=input_size)
- tdsa_2head (2 heads, attn_dim=input_size)
- tdsa_noncausal (no causal mask — control)
...
from lnn.core.tdsa_cfc import TimeDomainSelfAttentionCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_msdc_cfc.py)
# ---------------------------------------------------------------------------
...
def make_tdsa(input_size, hidden_size, output_size, num_layers=2):
    return TimeDomainSelfAttentionCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_tdsa_2head(input_size, hidden_size, output_size, num_layers=2):
    return TimeDomainSelfAttentionCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_tdsa_noncausal(input_size, hidden_size, output_size, num_layers=2):
    return TimeDomainSelfAttentionCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "tdsa": make_tdsa,
    "tdsa_2head": make_tdsa_2head,
    "tdsa_noncausal": make_tdsa_noncausal,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_tdsa_cfc.json")
    ...
    print(f"=== Round 152 TDSA-CfC Bench ===")
