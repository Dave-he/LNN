"""Round 170 — bench for PerFeatureH-CfC (per-feature β on h-side) (PRD #10-132).

30 cells: 5 conds × 3 datasets × 2 seeds × 30 epochs:
- pfh_h3_finer: 3-layer, Kx=5, Kh=3, per-feature β on h-side, base β=0.5 (sigmoid raw=0)
- pfh_h3_k6:   3-layer, Kx=6, Kh=3, per-feature β on h-side
- pfh_h4_wide: 3-layer, Kx=5, Kh=4, per-feature β on h-side
- pfh_h2_const: 3-layer, Kx=5, Kh=2, per-feature β on h-side (round 162 control)
- pfh_h5:      3-layer, Kx=5, Kh=5, per-feature β on h-side
...
# ---------------------------------------------------------------------------
# Data generators (mirror round 165-169)
# ---------------------------------------------------------------------------
...
CONDS = {
    "pfh_h3_finer": make_pfh_h3_finer,
    "pfh_h3_k6": make_pfh_h3_k6,
    "pfh_h4_wide": make_pfh_h4_wide,
    "pfh_h2_const": make_pfh_h2_const,
    "pfh_h5": make_pfh_h5,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_perfeature_h_cfc.json")
    ...
    print(f"=== Round 170 PerFeatureH-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, lr={args.lr}")
    print(f"5 conds × 3 datasets × 2 seeds × 30 epochs = 30 cells")
    print(f"Per-feature learned β on h-side, building on round 169 (Kh=3)")
