"""Round 151 — bench for Multi-Scale Dilated Conv CfC (MSDC-CfC) (PRD #10-113).

24 cells: 4 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- msdc_sum (3 dilations summed, then concat with x)
- msdc_concat (3 dilations concatenated with x)
...
from lnn.core.msdc_cfc import MultiScaleDilatedConvCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_lino_cfc.py)
# ---------------------------------------------------------------------------
...
def make_msdc_sum(input_size, hidden_size, output_size, num_layers=2):
    return MultiScaleDilatedConvCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_msdc_concat(input_size, hidden_size, output_size, num_layers=2):
    return MultiScaleDilatedConvCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_msdc_single(input_size, hidden_size, output_size, num_layers=2):
    return MultiScaleDilatedConvCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "msdc_sum": make_msdc_sum,
    "msdc_concat": make_msdc_concat,
    "msdc_single": make_msdc_single,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_msdc_cfc.json")
    ...
    print(f"=== Round 151 MSDC-CfC Bench ===")
