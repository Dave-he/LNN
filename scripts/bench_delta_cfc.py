"""Round 155 — bench for DELTA-CfC (Hidden State Delta Augmentation) (PRD #10-117).
...
from lnn.core.delta_cfc import DeltaCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_mono_cfc.py)
# ---------------------------------------------------------------------------
...
def make_delta_concat(input_size, hidden_size, output_size, num_layers=2):
    return DeltaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_delta_proj(input_size, hidden_size, output_size, num_layers=2):
    return DeltaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_delta_gated(input_size, hidden_size, output_size, num_layers=2):
    return DeltaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_delta_concat_input(input_size, hidden_size, output_size, num_layers=2):
    return DeltaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
        delta_mode="concat_input",
        return_sequences=True,
    )
...
CONDS = {
    "cfc": make_cfc,
    "delta_concat": make_delta_concat,
    "delta_proj": make_delta_proj,
    "delta_gated": make_delta_gated,
    "delta_concat_input": make_delta_concat_input,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_delta_cfc.json")
    ...
    print(f"=== Round 155 DELTA-CfC Bench ===")
