"""Round 157 — bench for LearnedBeta-CfC (per-feature learnable β EMA) (PRD #10-119).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- lb_concat ([x, ema], 2D input, learnable β)
- lb_gate (α·x + (1-α)·ema)
- lb_diff ([x, ema - x], 2D input, learnable β)
- lb_ema_only (ema only, control)

Datasets: sin_irr, structured_irr, random_irr (D=2, T=32, missing_rate=0.3).
β initialized to 0.9 via sigmoid (beta_raw=2.197).
"""
...
from lnn.core.learned_beta_cfc import LearnedBetaCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_ema_x_cfc.py)
# ---------------------------------------------------------------------------
...
def make_lb_concat(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_lb_gate(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_lb_diff(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_lb_ema_only(input_size, hidden_size, output_size, num_layers=2):
    return LearnedBetaCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "lb_concat": make_lb_concat,
    "lb_gate": make_lb_gate,
    "lb_diff": make_lb_diff,
    "lb_ema_only": make_lb_ema_only,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_learned_beta_cfc.json")
    ...
    print(f"=== Round 157 LearnedBeta-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, L={L}, lr={args.lr}, beta_init=0.9")
