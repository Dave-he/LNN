"""Round 136 — Bench Zoneout CfC on 3 datasets.

Tests whether the Zoneout regularizer from
arXiv:1606.01305 (Krueger et al. 2016, ICLR 2017, "Zoneout:
Regularizing RNNs by Preserving Hidden States") helps our 1D
...
- zoneout_low (p=0.1) — light Zoneout
- zoneout_med (p=0.3) — moderate Zoneout
"""
...
from lnn.core.zoneout_cfc import ZoneoutCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "zoneout_low":
        return ZoneoutCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, p_zoneout=0.1, return_sequences=True,
        )
    elif cond == "zoneout_med":
        return ZoneoutCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, p_zoneout=0.3, return_sequences=True,
        )
    else:
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 30,
    ...
    net = make_net(cond)
    if cond != "cfc":
        # Zoneout is train-only; ensure train mode.
        net.train()
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_zoneout_cfc.json")
    ...
    print(f"=== Round 136 Zoneout bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
