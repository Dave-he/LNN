#!/usr/bin/env python3
"""Benchmark for STEWithEntropy hidden=128 (round 271).

Tests whether r270's STRICT WIN continues to compound at h=128,
or whether h=64 is the saturation point.

The hypothesis (PRD #10-108): h=128 may continue to improve
(distribution property keeps gaining) or saturate (capacity
limit reached).

Modes (3 total):
  * ste_entropy_h32   — r270 reference
  * ste_entropy_h64   — r270 production
  * ste_entropy_h128  — NEW

Hypotheses (PRD #10-108):

  H1: prod_h128 ≥ prod_h64 on structured (compounds further)
  H2: prod_h128 ≈ prod_h64 (saturates)
...
# ---------------------------------------------------------------------------
# Toy data generators (match r270 bench protocol)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_hidden_128_bench.json"
    )
