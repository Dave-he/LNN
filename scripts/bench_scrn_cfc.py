"""Round 146 — Bench Slow Context RNN CfC on 3 datasets.

Tests whether a parallel slow context EMA stream helps our 1D time-series modelling.

Cells: 5 conditions × 3 datasets × 2 seeds = 30 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- scrn_alpha_05 (α=0.5, short memory)
- scrn_alpha_08 (α=0.8)
- scrn_alpha_095 (α=0.95, default long memory)
- scrn_alpha_099 (α=0.99, very long memory)
"""
...
from lnn.core.scrn_cfc import SCRNCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "scrn_alpha_05":
        return SCRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.5, return_sequences=True,
        )
    elif cond == "scrn_alpha_08":
        return SCRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.8, return_sequences=True,
        )
    elif cond == "scrn_alpha_095":
        return SCRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.95, return_sequences=True,
        )
    elif cond == "scrn_alpha_099":
        return SCRNCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, alpha_init=0.99, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_scrn_cfc.json")
    ...
    print(f"=== Round 146 SCRN-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
