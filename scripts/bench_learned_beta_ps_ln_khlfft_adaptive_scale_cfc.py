"""Round 224 — bench for AdaptiveScaleSpectralCfC (PRD #10-186, 2026-06-17)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_adaptive_scale_cfc import make_lbps_lnkhlfft_adaptive_scale_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    elif cond == "perlayer_234":
        model = make_lbps_lnkhlfft_perlayer_234_5_3_2(D, hidden, 1)
    elif cond == "adaptive_scale":
        model = make_lbps_lnkhlfft_adaptive_scale_5_3_2(D, hidden, 1)
    else:
...
def main():
    conds = ["cf", "4spectralbiasdrop", "perlayer_234", "adaptive_scale"]
    seeds = [0, 1]
    rows = []
    ...
    out = {
        "round": 224, "date": "2026-06-17",
        "config": "lbps_lnkhlfft_adaptive_scale_5_3_2 (per-step adaptive 2/3/4 branch) vs r223 (per-layer) vs r216 (uniform 4) vs cf",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_adaptive_scale_cfc.json"
