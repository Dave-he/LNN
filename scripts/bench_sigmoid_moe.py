"""Round 116 — Sigmoid Routing MoE bench (PRD #10-78).

Compares sigmoid-based MoE routing (Qwen2-MoE style, arXiv:2407.10671)
vs plain CfC and FAME on 3 datasets.  Tests whether replacing softmax
with sigmoid (no normalization) gives a stable dense routing signal in
low-D time-series.
...
Conditions:
  ...
  - sigmoid_k3_dense:  Sigmoid MoE K=3 dense (no top-K)
  - sigmoid_k3_t1:   Sigmoid MoE K=3 top_k=1
  - sigmoid_k3_t2:   Sigmoid MoE K=3 top_k=2
"""
...
from lnn.core.sigmoid_moe import SigmoidMoECfCNetwork, sigmoid_moe_utilization


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-115)
# ---------------------------------------------------------------------------
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond.startswith("sigmoid_"):
        # Parse n_experts and top_k
        # e.g., sigmoid_k3_dense, sigmoid_k3_t1, sigmoid_k3_t2
        parts = cond.split("_")
        k = int(parts[1][1:])
        if parts[2] == "dense":
            t = 0
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    # Sigmoid MoE utilization diagnostic
    routing_entropy = 0.0
    expert_util = []
    if cond.startswith("sigmoid_"):
        with torch.no_grad():
            _ = net(X_train)
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_sigmoid_moe.json")
    ...
    print(f"=== Round 116 Sigmoid MoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
