"""Round 137 — Bench 1D Convolutional Input Preprocessing for CfC.

Tests whether 1D causal convolution preprocessing BEFORE the CfC
recurrent step helps our 1D time-series modelling. Inspired by
...
- conv_k3 (1D causal conv with kernel_size=3, identity init)
- conv_k5 (1D causal conv with kernel_size=5, identity init)
"""
...
from lnn.core.conv_cfc import ConvCfCStackedNetwork
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_conv_cfc.json")
    ...
    print(f"=== Round 137 Conv-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
