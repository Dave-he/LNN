#!/usr/bin/env python3
"""Benchmark for STEWithEntropy large-λ sweep (round 268).

Extends r267's bench with larger λ values (1.0, 10.0, 100.0) to
test whether the monotonic trend on structured continues or
saturates.

Modes (6 total, extending r267):
  * ste_baseline          — r265 no-reg reference (λ=0)
  * ste_entropy_small     — r267 reference (λ=0.01)
  * ste_entropy_medium    — r267 best (λ=0.1, won 6.7× on structured)
  * ste_entropy_large     — NEW (λ=1.0)
  * ste_entropy_xl        — NEW (λ=10.0)
  * ste_entropy_xxl       — NEW (λ=100.0)

Hypotheses (PRD #10-105):

  H1: λ=1.0 is at least as good as λ=0.1 on structured.
  H2: λ=10.0 or 100.0 finds the global optimum.
  H3: λ ≥ 10.0 starts to hurt toy_sin/random.
  H4: Soft-mask entropy drops to < 10% of max entropy at λ ≥ 1.0.
  H5: Logit std continues to grow with λ (no collapse).
...
# ---------------------------------------------------------------------------
# Toy data generators (match r267 bench protocol)
# ---------------------------------------------------------------------------
...
class WrappedSTEWithEntropy(nn.Module):
    def __init__(self, input_size, hidden_size, entropy_lambda, density=0.3, ste_temperature=1.0):
        super().__init__()
...
# ---------------------------------------------------------------------------
# Mode definitions (extending r267)
# ---------------------------------------------------------------------------
MODES = {
    "ste_baseline": dict(entropy_lambda=0.0),
    "ste_entropy_small": dict(entropy_lambda=0.01),
    "ste_entropy_medium": dict(entropy_lambda=0.1),
    "ste_entropy_large": dict(entropy_lambda=1.0),
    "ste_entropy_xl": dict(entropy_lambda=10.0),
    "ste_entropy_xxl": dict(entropy_lambda=100.0),
}


def make_model(mode_cfg: dict, d_in: int, hidden_size: int) -> nn.Module:
    enc = WrappedSTEWithEntropy(
        d_in, hidden_size, entropy_lambda=mode_cfg["entropy_lambda"]
    )
    return SeqModel(enc, hidden_size, entropy_lambda=mode_cfg["entropy_lambda"])
...
def collect_diagnostics(model: SeqModel) -> dict:
    ...
    # Hard mask top-1 dominance: fraction of rows where one
    # element has > 80% of the row sum (delta-like).
    ...
    return {
        ...
        "entropy_loss_value": cell.entropy_value() * cell.entropy_lambda,
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_entropy_large_lambda_bench.json"
    )
    ...
    for mode_name in args.modes:
        ...
        for ds_name in args.datasets:
            for seed in args.seeds:
                ...
                print(
                    f"[bench] mode={mode_name:20s} ds={ds_name:10s} "
    ...
    print(f"{'mode':20s} | {'toy_sin':>10s} | {'structured':>10s} | {'random':>10s} | mean")
    for mode_name in args.modes:
        cols = []
