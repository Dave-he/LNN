#!/usr/bin/env python3
"""Benchmark for STEWithEntropy temperature sweep (round 269).

Tests whether smaller ste_temperature (sharper sigmoid) improves
over the r267/r268 production setting (τ=1.0, λ=0.1).

The hypothesis (PRD #10-106): smaller τ → sharper sigmoid →
lower entropy floor → entropy reg can concentrate more.

Modes (5 total):
  * ste_baseline_t1_l0        — r265 reference (τ=1.0, λ=0)
  * ste_entropy_t1_l0p1       — r267 best (τ=1.0, λ=0.1)
  * ste_entropy_t0p5_l0p1     — NEW (τ=0.5)
  * ste_entropy_t0p3_l0p1     — NEW (τ=0.3)
  * ste_entropy_t0p1_l0p1     — NEW (τ=0.1, very sharp)

Hypotheses (PRD #10-106):

  H1: Smaller τ at λ=0.1 ≥ τ=1.0 on structured.
  H2: Soft-mask entropy drops below 96% of max with smaller τ.
  H3: Logit std SATURATES at lower value with smaller τ.
  H4: Best τ is not the smallest (τ=0.3 sweet spot).
...
# ---------------------------------------------------------------------------
# Toy data generators (match r268 bench protocol)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
MODES = {
    "ste_baseline_t1_l0": dict(ste_temperature=1.0, entropy_lambda=0.0),
    "ste_entropy_t1_l0p1": dict(ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_t0p5_l0p1": dict(ste_temperature=0.5, entropy_lambda=0.1),
    "ste_entropy_t0p3_l0p1": dict(ste_temperature=0.3, entropy_lambda=0.1),
    "ste_entropy_t0p1_l0p1": dict(ste_temperature=0.1, entropy_lambda=0.1),
}
...
def collect_diagnostics(model: SeqModel) -> dict:
    ...
    return {
        "ste_temperature": cell.ste_temperature,
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_temperature_sweep_bench.json"
    )
