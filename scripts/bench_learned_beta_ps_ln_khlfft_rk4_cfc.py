"""Round 198 — RK4-CfC bench (PRD #10-160, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_rk4_cfc import make_lbps_lnkhlfft_rk4_5_3_2


# Dataset generators (consistent with r187-r197)
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_rk4_5_3_2(
            input_size=2, hidden_size=hidden, output_size=1,
        )
    else:
...
def main():
    ...
    for ds_name in DATASETS:
        for cond in conds:
            for seed in [0, 1]:
                ...
                rows.append(row)
                print(f"  {ds_name:18s} {cond:4s} seed={seed} "
                      f"test_mse={row['test_mse']:.4f} time={row['elapsed_s']}s")
    # Aggregate
    print("\n=== AGGREGATE (mean over seeds) ===")
    print(f"{'cond':6s} {'sin_irr':10s} {'struct_irr':10s} {'random_irr':10s} {'mean':10s} {'Δ':6s}")
    cf_means = {}
    for cond in conds:
        per_ds = {}
        for ds_name in DATASETS:
        ...
        if cond == "cf":
            cf_means = {ds: per_ds[ds] for ds in DATASETS}
            cf_mean = m
            delta = "—"
        else:
    ...
    out = {
        "round": 198,
        "date": "2026-06-16",
        "config": "lbps_lnkhlfft_rk4_5_3_2 vs lbps_lnkhlfft_5_3_2",
        "epochs": 30,
        "seeds": [0, 1],
        "n_cells": len(rows),
        "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_rk4_cfc.json"
