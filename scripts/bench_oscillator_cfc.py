"""Round 128 — Bench OscillatorCfC vs CfC baseline on 3 datasets.

Tests whether the damped harmonic oscillator closed-form cell
(arXiv:2602.12139, Shende et al. 2026) matches or beats CfC on
the 3 standard irregular-time-series datasets.

Cells: 2 models (CfC, OscillatorCfC) × 3 datasets × 2 seeds = 12 cells
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "oscillator_cfc":
        return OscillatorCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
            omega_init_lo=0.5, omega_init_hi=5.0,  # narrower band than paper
            zeta_init=-1.5,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_oscillator_cfc.json")
    ...
    print(f"=== Round 128 OscillatorCfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
