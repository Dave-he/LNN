"""Round 204 — SpectralDropoutHigh-CfC bench (PRD #10-166, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_specdropout_cfc import make_lbps_lnkhlfft_specdropout_5_3_2
from lnn.core.learned_beta_ps_ln_khlfft_specdropout_high_cfc import make_lbps_lnkhlfft_specdrophigh_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    elif cond == "specdrop_low":  # r203 p=0.3
        model = make_lbps_lnkhlfft_specdropout_5_3_2(
            input_size=2, hidden_size=hidden, output_size=1, dropout_p=0.3,
        )
    elif cond == "specdrop_high":  # r204 p=0.5
        model = make_lbps_lnkhlfft_specdrophigh_5_3_2(
            input_size=2, hidden_size=hidden, output_size=1, dropout_p=0.5,
        )
    else:
...
def main():
    ...
    conds = ["cf", "specdrop_low", "specdrop_high"]
    rows = []
    for ds_name in DATASETS:
        for cond in conds:
            for seed in [0, 1]:
                ...
                rows.append(row)
                print(f"  {ds_name:18s} {cond:14s} seed={seed} "
                      f"test_mse={row['test_mse']:.4f} time={row['elapsed_s']}s", flush=True)
    print("\n=== AGGREGATE (mean over seeds) ===", flush=True)
    print(f"{'cond':14s} {'sin_irr':10s} {'struct_irr':10s} {'random_irr':10s} {'mean':10s} {'Δ':6s}", flush=True)
    cf_mean = None
    for cond in conds:
        per_ds = {}
        for ds_name in DATASETS:
        ...
        print(f"{cond:14s} {per_ds['sin_irr']:.4f}     {per_ds['structured_irr']:.4f}     "
              f"{per_ds['random_irr']:.4f}     {m:.4f}     {delta}", flush=True)
    out = {
        "round": 204,
        "date": "2026-06-16",
        "config": "lbps_lnkhlfft_specdrophigh_5_3_2 (p=0.5) vs specdropout_5_3_2 (p=0.3) vs baseline",
        "epochs": 30,
        "seeds": [0, 1],
        "n_cells": len(rows),
        "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_specdropout_high_cfc.json"
