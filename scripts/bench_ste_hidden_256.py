#!/usr/bin/env python3
"""Benchmark for STEWithEntropy hidden=256 (round 272).

Tests whether r271's STRICT WIN continues at h=256, or whether
h=128 is the saturation point.

The hypothesis (PRD #10-109): power-law from r271 predicts
test_mse ≈ 0.000060 at h=256.

Modes (4 total):
  * ste_entropy_h64    — r271 reference
  * ste_entropy_h128   — r271 production
  * ste_entropy_h192   — NEW intermediate
  * ste_entropy_h256   — NEW find saturation

Hypotheses (PRD #10-109):

  H1: prod_h256 ≥ prod_h128 on structured (compounds further)
  H2: prod_h256 ≈ prod_h128 (saturates)
...
# ---------------------------------------------------------------------------
# Toy data generators (match r271 bench protocol)
# ---------------------------------------------------------------------------
...
MODES = {
    ...
    "ste_entropy_h128": dict(hidden_size=128, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_h192": dict(hidden_size=192, ste_temperature=1.0, entropy_lambda=0.1),
    "ste_entropy_h256": dict(hidden_size=256, ste_temperature=1.0, entropy_lambda=0.1),
}
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_hidden_256_bench.json"
    )
