"""Round 194 — bench for combined input+hidden noise augmentation (PRD #10-156)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_xhnoise_cfc import make_lbps_lnkhlfft_xhnoise_5_3_2
...
def run_dataset(name, make_fn, n_seeds=2, n_epochs=30, hidden=12):
    ...
    conds = [
        ...
        ("lbps_lnkhlfft_input05", lambda: make_lbps_lnkhlfft_noise_5_3_2(2, hidden, 1, noise_sigma=0.05)),
        ("lbps_lnkhlfft_hnoise05", lambda: make_lbps_lnkhlfft_hnoise_5_3_2(2, hidden, 1, hnoise_sigma=0.05)),
        ("lbps_lnkhlfft_xhnoise05", lambda: make_lbps_lnkhlfft_xhnoise_5_3_2(2, hidden, 1, noise_sigma=0.05, hnoise_sigma=0.05)),
    ]
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "results", "bench_learned_beta_ps_ln_khlfft_xhnoise_cfc.json")
    ...
    for ds, rows in out["datasets"].items():
        print(f"\n{ds}:")
        for cname in ["lbps_lnkhlfft_5_3_2_mse", "lbps_lnkhlfft_input05", "lbps_lnkhlfft_hnoise05", "lbps_lnkhlfft_xhnoise05"]:
