#!/usr/bin/env python3
"""Benchmark for PredictabilityGatedLiquidTauCfCCell (round 278).

Tests whether a parameter-free predictability gate on the liquid τ
contribution FIXES round 277's failure mode (liquid τ hurts +106% on
pure noise) while keeping its wins on predictable data.

Three modes (the full story):
  * static_tau    — r267 production (static per-neuron τ)
  * liquid_tau    — r277 (input-dependent τ, ungated)
  * gated_liquid  — r278 NEW (predictability-gated liquid τ)

Shared params match r267-r277 production:
...
# ---------------------------------------------------------------------------
# Toy data generators (identical to r276/r277)
# ---------------------------------------------------------------------------
...
MODES = {
    ...
    "gated_liquid": dict(kind="gated", pred_gate_beta=4.0, ema_gamma=0.5),
}
...
def main():
    ...
    parser.add_argument("--out", type=str,
                        default="analysis/pred_gated_liquid_tau_bench.json")
    ...
    for ds in args.datasets:
        ...
        for m in ("liquid_tau", "gated_liquid"):
            v = summary.get((m, ds), [])
            if v:
                vm = sum(v) / len(v)
