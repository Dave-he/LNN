"""Round 189 — bench for Bures-Wasserstein loss + CfC (PRD #10-151).

12 cells: 2 conds × 3 datasets × 2 seeds × 30 epochs:
- lbps_lnkhlfft_5_3_2_mse: round 187 winner, pure MSE loss (baseline)
- lbps_lnkhlfft_5_3_2_distdf: same model, BW + MSE combined loss (γ=0.5)
...
from lnn.core.bures_wasserstein_loss import combined_distdf_loss
...
def make_random_irr(B=32, T=32, D=2, missing_rate=0.3, seed=0):
    ...
    # Use only first feature as prediction target (D_out=1 to match model)
...
def train_and_eval(model, x_train, y_train, x_eval, y_eval, epochs, lr, loss_type, gamma=0.5):
    """Train with MSE or DistDF (γ*BW + (1-γ)*MSE) and eval with MSE."""
    ...
    for _ in range(epochs):
        ...
        elif loss_type == "distdf":
            loss = combined_distdf_loss(y_train, out, gamma=gamma)
        else:
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_bures_wasserstein_loss_cfc.json")
    ...
    print(f"=== Round 189 DistDF (Bures-Wasserstein) Loss Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, lr={args.lr}, gamma={args.gamma}")
    print(f"2 conds × 3 datasets × 2 seeds × 30 epochs = 12 cells")

    CONDS = [
        ("lbps_lnkhlfft_5_3_2_mse", "mse", 0.0),
        ("lbps_lnkhlfft_5_3_2_distdf", "distdf", args.gamma),
    ]
