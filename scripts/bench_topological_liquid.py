#!/usr/bin/env python3
"""Henry Hub validation for r297: TopologicalLiquidCfCCell.

Tests whether neuron-wise sparse graph-structured recurrent connections
improve Henry Hub overall test MSE vs the r295 default blend_gated cell.
"""
...
from lnn.core.topological_liquid_cfc import (  # noqa: E402
    TopologicalLiquidCfCCell,
)
...
# (mode_name, cell_factory)
def make_blend_gated():
    return BlendGatedLiquidTauCfCCell(
        input_size=1, hidden_size=128, density=0.3,
        liquid_tau_strength=1.0, pred_gate_beta=4.0, ema_gamma=0.5,
        gate_mode="blend",  # r295 default already in effect
    )


def make_topo(n_incoming):
    return TopologicalLiquidCfCCell(
        input_size=1, hidden_size=128, n_incoming=n_incoming,
        pred_gate_beta=4.0, ema_gamma=0.5, gate_mode="blend",
        decorr_lambda=1e-5,  # r295 default
    )


MODES = {
    "static_tau": ("static", None),
    "blend_gated_default": ("blend", None),
    "topo_n4": ("topo", 4),
    "topo_n8": ("topo", 8),
    "topo_n16": ("topo", 16),
    "topo_n32": ("topo", 32),
...
def make_model(cfg):
    kind, n_incoming = cfg
    if kind == "static":
        cell = STEWithEntropy(input_size=1, hidden_size=128, density=0.3,
                               ste_temperature=1.0, entropy_lambda=0.1)
    elif kind == "blend":
        cell = make_blend_gated()
    else:  # topo
        cell = make_topo(n_incoming)
    return SeqModel(cell, 128)
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default="analysis/topological_liquid_bench.json")
    args = ap.parse_args()
    ...
    for mode in args.modes:
        ...
        for seed in args.seeds:
            ...
            results["cells"].append({
                "mode": mode, "seed": seed, "n_params": n_params,
            ...
            print(f"[bench] {mode:22s} s{seed} "
                  f"params={n_params:>6d} mse={out['test_mse']:.5f} "
                  f"hi_vol={mse_hi:.5f} ({el:.1f}s)")
    ...
    params_d = agg("n_params")

    print("\n[bench] mean test MSE — overall | high-vol | n_params:")
    print(f"{'mode':22s} | {'overall':>10s} | {'hi_vol':>10s} | {'params':>8s}")
    for mode in args.modes:
        all_mse = sum(mse_all.get(mode, [])) / max(len(mse_all.get(mode, [])), 1)
        hi = sum(mse_hi.get(mode, [])) / max(len(mse_hi.get(mode, [])), 1)
        n_p = sum(params_d.get(mode, [])) / max(len(params_d.get(mode, [])), 1)
        print(f"{mode:22s} | {all_mse:>10.5f} | {hi:>10.5f} | {n_p:>8.0f}")

    print("\n[bench] Δ%% vs blend_gated_default (negative=better):")
    base_all = sum(mse_all.get("blend_gated_default", [])) / max(
        len(mse_all.get("blend_gated_default", [])), 1)
    base_hi = sum(mse_hi.get("blend_gated_default", [])) / max(
        len(mse_hi.get("blend_gated_default", [])), 1)
    print(f"  baseline blend_gated: overall={base_all:.5f} hi_vol={base_hi:.5f}")
    for m in ("topo_n4", "topo_n8", "topo_n16", "topo_n32"):
