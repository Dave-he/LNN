#!/usr/bin/env python3
"""Benchmark for EmaGatedPulseCfCCell (round 288).

Round 288 attacks the root cause of the r284-r287 trade-off curve:
the per-step gate `g_t` dips during gaps, breaking the "continuous
endogenous rhythm" claim. The fix is **EMA-smoothing** the gate
before thresholding:

    `g_ema_t = α · g_t + (1-α) · g_ema_{t-1}`
    `pulse = (g_ema_t > τ) · A · sin(...)`

On structured + gap: g_t dips momentarily but g_ema stays high →
pulse fires continuously through the gap.
On noise: g_t is consistently low → g_ema collapses → mask stays off.
...
Hypotheses:
  ...
  H5 (ema_alpha=1.0 ≡ r287) — unit-tested.
  H6 (threshold=0 ≡ r284) — unit-tested.

Modes:
  * static_tau         — r267 baseline
  * blend_gated        — r280 (primary baseline)
  * pulse_sin          — r284 (unconditional pulse)
  * binary_tau_05      — r287 (binary gate, τ=0.5)
  * ema_tau_03_a03     — r288 EMA α=0.3 τ=0.3
  * ema_tau_05_a03     — r288 EMA α=0.3 τ=0.5
  * ema_tau_05_a05     — r288 EMA α=0.5 τ=0.5
  * ema_tau_07_a03     — r288 EMA α=0.3 τ=0.7
"""
...
from lnn.core.ema_gated_pulse_cfc import (  # noqa: E402
    EmaGatedPulseCfCCell,
)
...
MODES = {
    ...
    "ema_a03_t03": dict(kind="ema", alpha=0.3, threshold=0.3),
    "ema_a03_t05": dict(kind="ema", alpha=0.3, threshold=0.5),
    "ema_a05_t05": dict(kind="ema", alpha=0.5, threshold=0.5),
    "ema_a03_t07": dict(kind="ema", alpha=0.3, threshold=0.7),
}
...
def make_model(cfg):
    if cfg["kind"] == "static":
        ...
        cell = EmaGatedPulseCfCCell(
            ...
            gate_mode="blend", pulse_strength=1.0, pulse_amp_init=0.1,
            pulse_mode="sin", state_phase=True,
            threshold=cfg["threshold"], ema_alpha=cfg["alpha"],
            g_ema_init=1.0, **_COMMON)
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default="analysis/ema_gated_pulse_bench.json")
    args = ap.parse_args()
    ...
    for d in args.datasets:
        ...
        for m in ("pulse_sin", "binary_tau_05", "ema_a03_t03",
                  "ema_a03_t05", "ema_a05_t05", "ema_a03_t07"):
            v = mse.get((m, d), [])
            if v:
                vm = sum(v) / len(v)
    ...
    print("\n[bench] H1/H2/H3 acceptance check (r288 → strict-positive?):")
    ...
    ema_modes = [m for m in args.modes if m.startswith("ema_")]
    h1_ok = False
    for m in ema_modes:
        v = mean("gr", m, "structured")
        ok = v <= r284_gr
        print(f"     {m}: gap_ratio={v:.2f}  {'OK' if ok else 'FAIL'}")
        if ok:
            h1_ok = True
    ...
    for m in ema_modes:
        ...
        ok = abs(delta) <= 5.0
        print(f"     {m}: Δ%%={delta:+.1f}%  {'OK' if ok else 'FAIL'}")
        if ok:
            h2_ok = True
    ...
    for m in ema_modes:
        v = mean("amp", m, "random")
        ok = v <= 0.20
        print(f"     {m}: amp={v:.3f}  {'OK' if ok else 'FAIL'}")
        if ok:
            h3_ok = True
