"""Round 143 — Bench Decoupled CfC + IndRNN-CfC on 3 datasets.

Tests the natural control experiment for round 142 (MI-CfC). Replaces
multiplicative combination with additive combination to isolate the
failure mode of round 142.

Cells: 3 conditions × 3 datasets × 2 seeds = 18 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- decoupled (additive: inter = W_x x + W_h h)
- indrnn (additive + element-wise recurrent: inter = W_x x + u * h)
"""
...
from lnn.core.decoupled_cfc import DecoupledCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return DecoupledCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, variant="decoupled", return_sequences=True,
        )
    elif cond == "indrnn":
        return DecoupledCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, variant="indrnn", return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_decoupled_cfc.json")
    ...
    print(f"=== Round 143 Decoupled-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
