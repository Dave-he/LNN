"""Round 163 — bench for HybridBeta-XH-CfC (Scalar β on h + Per-Feature β on x) (PRD #10-125).

24 cells: 4 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- hb_xh_scalar_h1:    Kx=1, Kh=1, per-feature β on x, scalar β=0.9 on h
- hb_xh_scalar_h2:    Kx=2, Kh=2, per-feature β on x, scalar β ∈ {0.7, 0.95} on h
- hb_xh_scalar_h2_3x: Kx=3, Kh=2, per-feature β on x, scalar β ∈ {0.7, 0.95} on h
- hb_xh_best:         Kx=3, Kh=2, per-feature β on x, scalar β ∈ {0.7, 0.95} on h, both diff

Datasets: sin_irr, structured_irr, random_irr (D=2, T=32, missing_rate=0.3).
β_init_x=2.197 (sigmoid(2.197) ≈ 0.9), betas_h=[0.7, 0.95].
"""
...
# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_learned_beta_xh_cfc.py)
# ---------------------------------------------------------------------------
...
def make_hb_xh_scalar_h1(input_size, hidden_size, output_size, num_layers=2):
...
def make_hb_xh_scalar_h2(input_size, hidden_size, output_size, num_layers=2):
    return HybridBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_hb_xh_scalar_h2_3x(input_size, hidden_size, output_size, num_layers=2):
    return HybridBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_hb_xh_best(input_size, hidden_size, output_size, num_layers=2):
    return HybridBetaXHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "hb_xh_scalar_h1": make_hb_xh_scalar_h1,
    "hb_xh_scalar_h2": make_hb_xh_scalar_h2,
    "hb_xh_scalar_h2_3x": make_hb_xh_scalar_h2_3x,
    "hb_xh_best": make_hb_xh_best,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_hybrid_beta_xh_cfc.json")
    ...
    print(f"=== Round 163 HybridBeta-XH-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, L={L}, lr={args.lr}")
    print(f"Per-feature β on x-side, SCALAR β on h-side (no h-side params)")
    print(f"β_init_x=2.197 (sigmoid(2.197) ≈ 0.9), betas_h=[0.7, 0.95]")
