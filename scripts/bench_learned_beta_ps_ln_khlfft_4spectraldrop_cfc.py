"""Round 214 — bench for 4ScaleSpectralDropCfC (PRD #10-176, 2026-06-16)."""
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_4spectraldrop_5_3_2(D, hidden, 1, dropout_p=0.2)
    else:
...
def main():
    conds = ["cf", "4spectral", "4spectraldrop"]
    seeds = [0, 1]
    rows = []
    for ds_name in DATASETS:
        for cond in conds:
            for seed in seeds:
                ...
                print(f"  {ds_name:18s} {cond:15s} s{seed}  test_mse={mse:.4f}  ({elapsed:.1f}s)")
    out = {
        "round": 214, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_4spectraldrop_5_3_2 (dropout p=0.2) vs 4spectral vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_4spectraldrop_cfc.json"
