"""Round 133 — Bench Hebbian Fast Weights CfC on 3 datasets.

Tests whether the Hebbian fast-weight mechanism from
arXiv:1610.06258 (Ba et al. NIPS 2016, "Using Fast Weights to
Attend to the Recent Past") helps our 1D time-series modelling.

Cells: 4 conditions × 3 datasets × 2 seeds = 24 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- fw_weak (lambda=0.95, eta=0.01) — slow fast weights
- fw_strong (lambda=0.9, eta=0.1) — strong fast weights
- fw_long (lambda=0.99, eta=0.05) — long-term fast weights
"""
...
from lnn.core.fast_weights_cfc import FastWeightsCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return FastWeightsCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, init_lambda=0.95, init_eta=0.01, return_sequences=True,
        )
    elif cond == "fw_strong":
        return FastWeightsCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, init_lambda=0.9, init_eta=0.1, return_sequences=True,
        )
    elif cond == "fw_long":
        return FastWeightsCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, init_lambda=0.99, init_eta=0.05, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_fast_weights_cfc.json")
    ...
    print(f"=== Round 133 FastWeights bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
