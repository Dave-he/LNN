"""Round 206 — bench for AttentionCfC (PRD #10-168, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_attn_cfc import make_lbps_lnkhlfft_attn_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_attn_5_3_2(D, hidden, 1)
    else:
    ...
    for ds_name in DATASETS:
        for cond in conds:
            for seed in seeds:
                ...
                print(f"  {ds_name:18s} {cond:6s} s{seed}  test_mse={mse:.4f}  ({elapsed:.1f}s)")
    out = {
        "round": 206, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_attn_5_3_2 (attention over h) vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_attn_cfc.json"
