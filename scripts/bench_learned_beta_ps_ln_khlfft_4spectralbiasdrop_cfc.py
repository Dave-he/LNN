"""Round 216 — bench for 4ScaleSpectralBiasDropCfC (PRD #10-178, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_4spectral_cfc import make_lbps_lnkhlfft_4spectral_5_3_2
from lnn.core.learned_beta_ps_ln_khlfft_4spectralbias_cfc import make_lbps_lnkhlfft_4spectralbias_5_3_2
from lnn.core.learned_beta_ps_ln_khlfft_4spectraldrop_cfc import make_lbps_lnkhlfft_4spectraldrop_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_4spectral_5_3_2(D, hidden, 1)
    elif cond == "4spectralbias":
        model = make_lbps_lnkhlfft_4spectralbias_5_3_2(D, hidden, 1)
    elif cond == "4spectraldrop":
        model = make_lbps_lnkhlfft_4spectraldrop_5_3_2(D, hidden, 1)
...
def main():
    conds = ["cf", "4spectral", "4spectralbias", "4spectraldrop", "4spectralbiasdrop"]
    seeds = [0, 1]
    rows = []
    for ds_name in DATASETS:
        for cond in conds:
            for seed in seeds:
                ...
                print(f"  {ds_name:18s} {cond:22s} s{seed}  test_mse={mse:.4f}  ({elapsed:.1f}s)")
    out = {
        "round": 216, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_4spectralbiasdrop_5_3_2 vs 4spectral vs bias vs drop vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_4spectralbiasdrop_cfc.json"
