"""Round 160 — bench for MultiBeta-H-CfC (Multi-Scale Hidden State EMA) (PRD #10-122).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- mbh_diff_2 (K=2 high-pass)
- mbh_concat_2 (K=2 low-pass)
- mbh_diff_3 (K=3 high-pass)
- mbh_concat_3 (K=3 low-pass)

Datasets: sin_irr, structured_irr, random_irr (D=2, T=32, missing_rate=0.3).
K=2: β ∈ {0.7, 0.95}, K=3: β ∈ {0.5, 0.9, 0.99}.
"""
...
from lnn.core.multi_beta_h_cfc import MultiBetaHCfCStackedNetwork
...
def make_mbh_diff_2(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_mbh_concat_2(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mbh_diff_3(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mbh_concat_3(input_size, hidden_size, output_size, num_layers=2):
    return MultiBetaHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "mbh_diff_2": make_mbh_diff_2,
    "mbh_concat_2": make_mbh_concat_2,
    "mbh_diff_3": make_mbh_diff_3,
    "mbh_concat_3": make_mbh_concat_3,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_multi_beta_h_cfc.json")
