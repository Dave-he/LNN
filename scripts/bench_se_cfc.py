"""Round 140 — Bench Squeeze-and-Excitation (SE) CfC on 3 datasets.

Tests whether per-channel input attention (SE-Net from Hu et al. 2017)
...
- se_concat (SE score from concat [x, h])
- se_input (SE score from input only)
"""
...
from lnn.core.se_cfc import SECfCStackedNetwork
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_se_cfc.json")
    ...
    print(f"=== Round 140 SE-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
