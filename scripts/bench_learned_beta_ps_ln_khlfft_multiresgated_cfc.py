"""Round 209 — bench for MultiResSpectralGatedCfC (PRD #10-171, 2026-06-16)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_multiresgated_cfc import make_lbps_lnkhlfft_multiresgated_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=12, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_multiresgated_5_3_2(D, hidden, 1)
    else:
    ...
    for ds_name in DATASETS:
        for cond in conds:
            for seed in seeds:
                ...
                print(f"  {ds_name:18s} {cond:10s} s{seed}  test_mse={mse:.4f}  ({elapsed:.1f}s)")
    out = {
        "round": 209, "date": "2026-06-16",
        "config": "lbps_lnkhlfft_multiresgated_5_3_2 (2-scale FFT) vs baseline",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_multiresgated_cfc.json"
