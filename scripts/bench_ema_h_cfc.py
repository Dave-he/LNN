"""Round 159 — bench for EMA-H-CfC (Hidden State EMA Augmentation) (PRD #10-121).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline)
- eh_concat ([h, ema_h], 2H input)
- eh_gate (α·h + (1-α)·ema_h)
- eh_diff ([h, ema_h - h], 2H input)
- eh_ema_only (ema_h only, control)
...
from lnn.core.ema_h_cfc import EMAHCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_multi_beta_cfc.py)
# ---------------------------------------------------------------------------
...
def make_eh_concat(input_size, hidden_size, output_size, num_layers=2):
    return EMAHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_eh_gate(input_size, hidden_size, output_size, num_layers=2):
    return EMAHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_eh_diff(input_size, hidden_size, output_size, num_layers=2):
    return EMAHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_eh_ema_only(input_size, hidden_size, output_size, num_layers=2):
    return EMAHCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    "cfc": make_cfc,
    "eh_concat": make_eh_concat,
    "eh_gate": make_eh_gate,
    "eh_diff": make_eh_diff,
    "eh_ema_only": make_eh_ema_only,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_ema_h_cfc.json")
    ...
    print(f"=== Round 159 EMA-H-CfC Bench ===")
    print(f"epochs={args.epochs}, seeds={args.seeds}, B={args.batch}, T={args.T}, H={H}, L={L}, lr={args.lr}, beta=0.9")
