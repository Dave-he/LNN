"""Round 228 — bench for ICFATCSpectralCfC (PRD #10-190, 2026-06-17)."""
...
from lnn.core.learned_beta_ps_ln_khlfft_icfatc_cfc import make_lbps_lnkhlfft_icfatc_5_3_2
...
def run_one_cell(ds_name, cond, seed, hidden=16, epochs=30):
    ...
    if cond == "cf":
        ...
        model = make_lbps_lnkhlfft_icfatc_5_3_2(D, hidden, 1)
    else:
...
def main():
    conds = ["cf", "4spectralbiasdrop", "fatc", "icfatc"]
    seeds = [0, 1]
    rows = []
    ...
    out = {
        "round": 228, "date": "2026-06-17",
        "config": "lbps_lnkhlfft_icfatc_5_3_2 (input-conditioned FATC mix) vs r225 (FATC) vs r216 vs cf",
        "epochs": 30, "seeds": seeds, "n_cells": len(rows), "rows": rows,
    }
    out_path = "results/bench_learned_beta_ps_ln_khlfft_icfatc_cfc.json"
