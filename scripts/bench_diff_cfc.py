"""Round 145 — Bench Difference Features CfC on 3 datasets.

Tests whether finite-difference input features help our 1D time-series modelling.

Cells: 5 conditions × 3 datasets × 2 seeds = 30 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- diff_concat_1 (input = [x, Δx])
- diff_concat_2 (input = [x, Δx, Δ²x])
- diff_only_1 (input = [Δx])
- diff_only_2 (input = [Δx, Δ²x])
"""
...
from lnn.core.diff_cfc import DiffCfCNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "diff_concat_1":
        return DiffCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            n_diff=1, mode="concat", num_layers=2, return_sequences=True,
        )
    elif cond == "diff_concat_2":
        return DiffCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            n_diff=2, mode="concat", num_layers=2, return_sequences=True,
        )
    elif cond == "diff_only_1":
        return DiffCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            n_diff=1, mode="diff_only", num_layers=2, return_sequences=True,
        )
    elif cond == "diff_only_2":
        return DiffCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            n_diff=2, mode="diff_only", num_layers=2, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_diff_cfc.json")
    ...
    print(f"=== Round 145 Diff-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
