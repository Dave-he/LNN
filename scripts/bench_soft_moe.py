"""Round 107 — Bench: Soft MoE routing on top of SETA (PRD #10-69).

Measures test_mse, routing entropy, and training stability for:
  - 4 conditions: seta_only_shared (round 105 baseline), seta_soft_default,
                  seta_soft_cosine, seta_soft_d8
  - 3 datasets: sin_irr, structured_irr, random_irr
  - 1 K setting: S=2+U=3 (K=5)
...
# Data generators (reused from rounds 102-106)
...
def build_model(cond, input_size, hidden_size, n_shared, n_unique, top_k):
    ...
    if cond == "seta_soft_default":
        return SoftMoESETAMoECfCNetwork(
            input_size=input_size, hidden_size=hidden_size,
            sdta_config=sdta_cfg, d_slot=16, normalize=False,
            n_queries=4, d_context=16, n_heads=4, output_size=input_size,
        )
    if cond == "seta_soft_cosine":
        return SoftMoESETAMoECfCNetwork(
            input_size=input_size, hidden_size=hidden_size,
            sdta_config=sdta_cfg, d_slot=16, normalize=True,
            n_queries=4, d_context=16, n_heads=4, output_size=input_size,
        )
    if cond == "seta_soft_d8":
        return SoftMoESETAMoECfCNetwork(
            input_size=input_size, hidden_size=hidden_size,
            sdta_config=sdta_cfg, d_slot=8, normalize=False,
            n_queries=4, d_context=16, n_heads=4, output_size=input_size,
        )
...
def run_cell(cond, dataset, n_shared, n_unique, top_k, seed, n_epochs,
    ...
    softmoe_std_list, softmoe_maxmin_list = [], []
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 0 and hasattr(model, "get_utilization"):
            ...
            if "softmoe_expert_norm_std" in util:
                softmoe_std_list.append(util["softmoe_expert_norm_std"])
                softmoe_maxmin_list.append(util["softmoe_expert_norm_max_min_ratio"])
    ...
    return {
        ...
        "softmoe_norm_std_mean": float(np.mean(softmoe_std_list)) if softmoe_std_list else 0.0,
        "softmoe_max_min_ratio_mean": float(np.mean(softmoe_maxmin_list)) if softmoe_maxmin_list else 0.0,
...
def main():
    ...
    p.add_argument("--out", default="results/bench_soft_moe.json")
    ...
    p.add_argument(
        "--conds", nargs="+",
        default=[
            "seta_only_shared", "seta_soft_default",
            "seta_soft_cosine", "seta_soft_d8",
        ],
    )
            ...
            for k_setting in args.K_topk:
                ...
                for seed in args.seeds:
                    ...
                    print(
                        ...
                        f"softmoe_std={r['softmoe_norm_std_mean']:.3f} "
                        f"max_min={r['softmoe_max_min_ratio_mean']:.2f} "
    ...
    for cond in args.conds:
        for dataset in args.datasets:
            for k_setting in args.K_topk:
                ...
                softmoe_std = np.mean([r["softmoe_norm_std_mean"] for r in rows])
                softmoe_max_min = np.mean([r["softmoe_max_min_ratio_mean"] for r in rows])
                ...
                print(
                    ...
                    f"softmoe_std={softmoe_std:.3f} max_min={softmoe_max_min:.2f} "
                    f"stable={stable}"
                )
