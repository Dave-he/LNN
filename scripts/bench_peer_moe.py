"""Round 119 — PEER MoE bench (PRD #10-81).

Compares PEER-style single-neuron experts (arXiv:2407.04153) with
product-key routing against plain CfC, FAME, Sigmoid, and LoRA on
3 datasets.  Tests whether a new linear expert family can match/be
the non-linear winners of the 91-118 audit.
...
Conditions:
  ...
  - peer_n8_pk:        PEER N=8 experts, top_k=2, product-key router
  - peer_n8_softmax:   PEER N=8 experts, top_k=2, softmax router
"""
...
from lnn.core.peer_moe import PEERCfCNetwork, peer_utilization
from lnn.core.sigmoid_moe import SigmoidMoECfCNetwork


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-118)
# ---------------------------------------------------------------------------
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    ...
    elif cond == "peer_n8_pk":
        return PEERCfCNetwork(
            input_size=input_size,
            hidden_size=hidden_size,
            output_size=output_size,
            num_layers=2,
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 30,
    ...
    # PEER utilization diagnostic
    routing_entropy = 0.0
    expert_util = []
    n_peer_params = 0
    n_active = 0
    if cond.startswith("peer_"):
        with torch.no_grad():
            _ = net(X_train)
        for cell in net.cells:
            if hasattr(cell, "last_expert_util") and cell.last_expert_util is not None:
                ...
                n_peer_params = int(diag["n_peer_params"])
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_peer_moe.json")
    ...
    print(f"=== Round 119 PEER MoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
