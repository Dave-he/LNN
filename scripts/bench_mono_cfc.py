"""Round 154 — bench for MONO-CfC (Monotonic Activation CfC) (PRD #10-116).

30 cells: 5 conds × 3 datasets × 2 seeds, 30 epochs:
- cfc (baseline, Tanh+Tanh)
- mono_g (g=Softplus, h=Tanh)
- mono_h (g=Tanh, h=Softplus)
- mono_both (g=Softplus, h=Softplus)
- mono_sig (g=Sigmoid, h=Sigmoid, control bounded)
...
from lnn.core.mono_cfc import MonoCfCStackedNetwork


# ---------------------------------------------------------------------------
# Data generators (mirror scripts/bench_film_cfc.py)
# ---------------------------------------------------------------------------
...
def make_mono_g(input_size, hidden_size, output_size, num_layers=2):
    return MonoCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
        num_layers=num_layers,
...
def make_mono_h(input_size, hidden_size, output_size, num_layers=2):
    return MonoCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mono_both(input_size, hidden_size, output_size, num_layers=2):
    return MonoCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
def make_mono_sig(input_size, hidden_size, output_size, num_layers=2):
    return MonoCfCStackedNetwork(
        input_size=input_size,
        hidden_size=hidden_size,
        output_size=output_size,
...
CONDS = {
    ...
    "mono_g": make_mono_g,
    "mono_h": make_mono_h,
    "mono_both": make_mono_both,
    "mono_sig": make_mono_sig,
}
...
def main():
    ...
    parser.add_argument("--out", type=str, default="results/bench_mono_cfc.json")
    ...
    print(f"=== Round 154 MONO-CfC Bench ===")
