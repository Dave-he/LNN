"""Round 139 — Bench Gated Linear Unit CfC on 3 datasets.

Tests whether per-feature input gating (LSTM-style input gate, GLU
from Dauphin 2017) helps our 1D time-series modelling.

Cells: 3 conditions × 3 datasets × 2 seeds = 18 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- glu_basic (per-feature sigmoid input gate)
...
from lnn.core.gated_linear_unit_cfc import GatedLinearUnitCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return GatedLinearUnitCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
        )
    elif cond == "glu_residual":
        # GLU + identity skip on input (LSTM-style: gated + identity).
        return GLUResidualCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
        )
    else:
...
class GLUResidualCfCStackedNetwork(nn.Module):
    """GLU + identity skip on input (LSTM-style: gated + identity)."""

    def __init__(self, input_size, hidden_size, output_size, num_layers=2, return_sequences=True):
        super().__init__()
        from lnn.core.gated_linear_unit_cfc import GatedLinearUnitCfCCell
        ...
        for l in range(num_layers):
            ...
            self.cells.append(GatedLinearUnitCfCCell(in_size, hidden_size))
        self.head = nn.Linear(hidden_size, output_size)

    def forward(self, x):
        B, T, _ = x.shape
        h = [torch.zeros(B, self.hidden_size, device=x.device, dtype=x.dtype) for _ in range(self.num_layers)]
        outputs = []
        for t in range(T):
            inp = torch.nan_to_num(x[:, t, :], nan=0.0)
            for li, cell in enumerate(self.cells):
                # GLU modulation + identity skip.
                x_gate = cell.input_gate(inp)
                x_gated = x_gate * inp + (1.0 - x_gate) * 0.0  # gated part only
                x_aug = x_gated + inp  # add identity skip
                combined = torch.cat([x_aug, h[li]], dim=-1)
                f = cell.f_gate(combined)
                g = cell.g_branch(combined)
                h_out = cell.h_branch(combined)
                decay = torch.sigmoid(-f * cell.time_scale)
                h[li] = decay * g + (1.0 - decay) * h_out
                inp = h[li]
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_gated_linear_unit_cfc.json")
    ...
    print(f"=== Round 139 GLU-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
