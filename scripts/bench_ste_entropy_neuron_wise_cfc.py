#!/usr/bin/env python3
"""Benchmark for STEWithEntropy (round 267).

Tests whether soft-mask entropy regularization improves over
the r265 STE baseline. The penalty is:

  L_total = L_task + λ × H(soft_mask_row)

Where H is per-row Shannon entropy of the row-softmaxed soft
sigmoid mask.

Modes (4 total):
  * ste_baseline       — r265 no-reg reference (entropy_lambda=0)
  * ste_entropy_tiny   — light entropy reg (λ=0.001)
  * ste_entropy_small  — moderate (λ=0.01)
  * ste_entropy_medium — strong (λ=0.1)
...
Hypotheses (PRD #10-104):
  ...
  H3: Entropy reg REDUCES soft-mask entropy (clearer concentration)
      without collapsing logits (cf r266 L1).
...
# ---------------------------------------------------------------------------
# Toy data generators (match r266 bench protocol)
# ---------------------------------------------------------------------------
...
MODES = {
    ...
    "ste_entropy_tiny": dict(entropy_lambda=0.001),
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="analysis/ste_entropy_neuron_wise_cfc_bench.json"
    )
