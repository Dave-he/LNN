"""Round 129 — Bench ELMCfC vs CfC baseline on 3 datasets.

Tests whether the Expressive Leaky Memory cell
(arXiv:2605.12049, Spieler, Martius, Levina 2026) with
multi-timescale memory units and high-pass filtered
output helps our recurrent 1D TS modelling.

Cells: 2 models (CfC, ELMCfC) × 3 datasets × 2 seeds = 12 cells
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        # Use ELM with d_m=4 (Pareto recipe: d_m ~ sqrt(N))
        return ELMCfCNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
            d_m=4, d_mlp=8, tau_m_lo=0.5, tau_m_hi=5.0, tau_r=1.0,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_elm_cfc.json")
    ...
    print(f"=== Round 129 ELMCfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
