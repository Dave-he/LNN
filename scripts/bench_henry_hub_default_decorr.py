#!/usr/bin/env python3
"""Henry Hub validation for r293 default decorrelation (round 293).

Confirms that when decorrelation is moved INSIDE the cell's extra_loss()
(rather than added in the bench wrapper), the SP result from r292
holds: blend_gated with default decorr_lambda=1e-4 should match the
opt-in r292 result (overall -0.3%, hi_vol -1.0%).
"""
...
class SeqModel(nn.Module):
    """Minimal wrapper: only the cell's extra_loss (no bench-side decor).

    This is the new default path: blend_gated with decorr_lambda=1e-4
    has decorrelation built into extra_loss().
    """
...
def make_model(cfg):
    if cfg["kind"] == "static":
        ...
        cell = BlendGatedLiquidTauCfCCell(
            ...
            gate_mode="blend", decorr_lambda=cfg["decorr"], **_COMMON)
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default="analysis/henry_hub_default_decorr_bench.json")
    args = ap.parse_args()
    ...
    print("\n[bench] Δ%% vs blend_old (r280 baseline):")
    base_all = sum(mse_all.get("blend_old", [])) / max(
        len(mse_all.get("blend_old", [])), 1)
    ...
    print(f"  Expected (r292 opt-in): overall Δ%≈{target_overall}%, "
          f"hi_vol Δ%≈{target_hi}%")
    print(f"  Got (r293 default):    overall Δ%={d_all:+.1f}%, "
          f"hi_vol Δ%={d_hi:+.1f}%")
    h8_ok = d_all <= 5.0 and d_hi <= 5.0
    print(f"  H8 (within 5% of baseline): "
          f"{'OK — r293 default reproduces r292 SP' if h8_ok else 'FAIL'}")
