"""Round 138 — Bench Sinusoidal Time Embedding CfC on 3 datasets.

Tests whether adding a sinusoidal time embedding to the input
(Vaswani 2017-style positional encoding) helps our 1D time-series
modelling. Inspired by Transformer positional encoding.

Cells: 3 conditions × 3 datasets × 2 seeds = 18 cells
Datasets: sin_irr, structured_irr, random_irr
Conditions:
- cfc (baseline, vanilla CfC)
- te_d4 (concat 4-dim sinusoidal time embedding)
- te_d8 (concat 8-dim sinusoidal time embedding)
"""
...
from lnn.core.sinusoidal_time_emb_cfc import SinusoidalTimeEmbCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return SinusoidalTimeEmbCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, time_emb_dim=4, return_sequences=True,
        )
    elif cond == "te_d8":
        return SinusoidalTimeEmbCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, time_emb_dim=8, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_sinusoidal_time_emb_cfc.json")
    ...
    print(f"=== Round 138 TE-CfC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
    ...
    print(f"{'cond':<10} | " + " | ".join(f"{d:<14}" for d in cells))
    for cond in conds:
        row = [cond]
        for dataset in cells:
    ...
    for cond in conds:
        ...
        print(f"  {cond:<10} n_params={params[0]}")
