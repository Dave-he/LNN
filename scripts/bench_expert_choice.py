"""Round 112 — Expert Choice routing bench (PRD #10-74).

Compares Expert Choice (EC) routing vs token-choice FAME on 3 datasets.
EC assigns each expert its top-k tokens, giving perfect load balance
by construction (no aux loss needed).

Cells: 3 datasets × 4 conditions × 2 seeds = 24 cells
Conditions:
  - baseline_cfc:  standard CfC, no MoE (control)
  - ec_dense:      EC with cap_k=T (every expert processes all tokens)
  - ec_half:       EC with cap_k_frac=0.5 (each expert processes 50% of tokens)
  - ec_quarter:    EC with cap_k_frac=0.25 (each expert processes 25% of tokens)
"""
...
# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-111)
# ---------------------------------------------------------------------------
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "baseline_cfc":
        ...
        return ExpertChoiceCfCNetwork(
            ...
            return_sequences=True,
            n_experts=4,
            cap_k=None,  # dense: each expert processes all tokens
        )
    elif cond == "ec_half":
        return ExpertChoiceCfCNetwork(
            input_size=input_size,
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 50,
    ...
    # Compute process fraction: how many tokens each expert processes
    # on average (averaged over experts and batch).
    process_frac = 1.0
    if cond.startswith("ec_") and cond != "ec_dense":
        if cond == "ec_half":
            process_frac = 0.5
        elif cond == "ec_quarter":
            process_frac = 0.25
...
CONDITIONS = ["baseline_cfc", "ec_dense", "ec_half", "ec_quarter"]
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_expert_choice.json")
    ...
    print(f"=== Round 112 EC bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
    ...
    for cond in conds:
        ...
        for d in cells:
            ...
            std = torch.tensor(vals).std().item() if len(vals) > 1 else 0.0
