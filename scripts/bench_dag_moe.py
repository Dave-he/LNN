"""Round 120 — DAG-MoE bench (PRD #10-82).

Compares DAG-MoE (structural aggregation, arXiv:2606.01062) with
plain CfC, FAME, Sigmoid, and LoRA on 3 datasets.  Tests whether the
DAG aggregation beats weighted summation in 1D time-series.

Cells: 3 datasets × 7 conditions × 2 seeds = 42 cells
Conditions:
  ...
  - dag_moe_k3_l2:     DAG-MoE K=3 L=2 (round 120, top_k=3)
  - dag_moe_k3_l1:     DAG-MoE K=3 L=1 (ablation, fewer iterations)
  - dag_moe_k4_l2_top2: DAG-MoE K=4 top_k=2 L=2 (sparse DAG)
"""
...
from lnn.core.dag_moe import DAGMoECfCNetwork, dag_moe_utilization


# ---------------------------------------------------------------------------
# Dataset generators (same as rounds 102-119)
# ---------------------------------------------------------------------------
...
def train_one(cond: str, dataset: str, seed: int, epochs: int = 30,
    ...
    # DAG-MoE utilization diagnostic
    n_dag_params = 0
    n_expert_params = 0
    n_total_params = sum(p.numel() for p in net.parameters())
    if cond.startswith("dag_moe"):
        for cell in net.cells:
            if hasattr(cell, "dag"):
                util = dag_moe_utilization(cell)
                n_dag_params = int(util["n_dag_params"])
                n_expert_params = int(util["n_expert_params"])
                break
    ...
    return {
        ...
        "n_expert_params": n_expert_params,
    }
...
def main():
    ...
    parser.add_argument("--output", type=str,
                        default="results/bench_dag_moe.json")
    ...
    print(f"=== Round 120 DAG-MoE bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
