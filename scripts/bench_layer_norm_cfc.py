"""Round 135 — Bench Layer Norm CfC on 3 datasets.

Tests whether the Layer Normalization mechanism from
arXiv:1607.06450 (Ba, Kiros, Hinton, 2016, "Layer Normalization")
...
- ln_cfc (LN applied to combined = [x, h] before linear projection)
- ln_cfc_affine (LN with learnable affine gamma, beta — default behavior)
"""
...
from lnn.core.layer_norm_cfc import LayerNormCfCStackedNetwork
...
def make_net(cond: str, input_size: int = 2, hidden_size: int = 16, output_size: int = 1):
    if cond == "cfc":
        ...
        return LayerNormCfCStackedNetwork(
            input_size=input_size, hidden_size=hidden_size, output_size=output_size,
            num_layers=2, return_sequences=True,
        )
    else:
...
def main():
    ...
    parser.add_argument("--output", type=str, default="results/bench_layer_norm_cfc.json")
    ...
    print(f"=== Round 135 LayerNorm bench: {len(cells)} datasets × {len(conds)} conds × {seeds} seeds = {total} cells, {epochs} epochs ===")
    t0 = time.time()
